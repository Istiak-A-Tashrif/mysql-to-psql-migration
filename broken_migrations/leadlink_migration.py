...
def get_leadlink_table_info():
    """Get MySQL table information for LeadLink"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `LeadLink`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting LeadLink table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'LeadLink'"
        ])
        ...
        print(f"Error getting LeadLink table info: {e}")
        return None, None, None

def convert_leadlink_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for LeadLink"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?LeadLink`?', f"CREATE TABLE "LeadLink"\', postgres_ddl)
        ...
        print(f"Error converting LeadLink DDL: {e}")
        return None
...
def create_leadlink_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating LeadLink table: {e}")
        return False
...
def create_leadlink_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating LeadLink indexes: {e}")
        return False
...
def create_leadlink_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating LeadLink foreign keys: {e}")
        return False
