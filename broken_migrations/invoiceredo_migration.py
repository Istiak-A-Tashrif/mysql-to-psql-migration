...
def get_invoiceredo_table_info():
    """Get MySQL table information for InvoiceRedo"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `InvoiceRedo`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting InvoiceRedo table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `InvoiceRedo`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'InvoiceRedo'"
        ])
        ...
        print(f"Error getting InvoiceRedo table info: {e}")
        return None, None, None

def convert_invoiceredo_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for InvoiceRedo"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?InvoiceRedo`?', f"CREATE TABLE "InvoiceRedo"\', postgres_ddl)
        ...
        print(f"Error converting InvoiceRedo DDL: {e}")
        return None
...
def create_invoiceredo_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating InvoiceRedo table: {e}")
        return False
...
def create_invoiceredo_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating InvoiceRedo indexes: {e}")
        return False
...
def create_invoiceredo_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating InvoiceRedo foreign keys: {e}")
        return False
