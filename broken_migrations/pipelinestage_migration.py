...
def get_pipelinestage_table_info():
    """Get MySQL table information for PipelineStage"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `PipelineStage`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting PipelineStage table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `PipelineStage`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'PipelineStage'"
        ])
        ...
        print(f"Error getting PipelineStage table info: {e}")
        return None, None, None

def convert_pipelinestage_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for PipelineStage"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?PipelineStage`?', f"CREATE TABLE "PipelineStage"\', postgres_ddl)
        ...
        print(f"Error converting PipelineStage DDL: {e}")
        return None
...
def create_pipelinestage_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating PipelineStage table: {e}")
        return False
...
def create_pipelinestage_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating PipelineStage indexes: {e}")
        return False
...
def create_pipelinestage_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating PipelineStage foreign keys: {e}")
        return False
