#!/usr/bin/env python3
"""
MarketingAutomationRule Table Migration Script
=============================

This script provides a complete 3-phase migration approach specifically for the MarketingAutomationRule table:
...
- Handles MarketingAutomationRule-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for MarketingAutomationRule table
- Large table with customer/marketingautomationrule data

Usage: 
    python marketingautomationrule_migration.py --phase=1
    python marketingautomationrule_migration.py --phase=2
    python marketingautomationrule_migration.py --phase=3
    python marketingautomationrule_migration.py --full
    python marketingautomationrule_migration.py --verify
"""
...
def get_marketingautomationrule_table_info():
    """Get complete MarketingAutomationRule table information from MySQL including constraints"""
    ...
    indexes = extract_marketingautomationrule_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_marketingautomationrule_foreign_keys_from_ddl(mysql_ddl)
...
def extract_marketingautomationrule_indexes_from_ddl(ddl):
    """Extract index definitions from MarketingAutomationRule table MySQL DDL"""
    indexes = []
    ...
    for match in matches:
        ...
        indexes.append({
            ...
            'table': 'MarketingAutomationRule'
        })
    
    return indexes

def extract_marketingautomationrule_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from MarketingAutomationRule table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to MarketingAutomationRule - handle multi-word actions like "SET NULL"
...
def convert_marketingautomationrule_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert MarketingAutomationRule table MySQL DDL to PostgreSQL DDL with MarketingAutomationRule-specific optimizations"""
    print(f" Converting MarketingAutomationRule table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_marketingautomationrule_column_definition(line, preserve_case)
            if processed_line:
...
def process_marketingautomationrule_column_definition(line, preserve_case):
    """Process a single column definition for MarketingAutomationRule table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for MarketingAutomationRule
    conversions = [
    ...
    # Handle specific MarketingAutomationRule issues: make first_name nullable to handle empty strings
...
def create_marketingautomationrule_table(mysql_ddl):
    """Create MarketingAutomationRule table in PostgreSQL"""
    postgres_ddl = convert_marketingautomationrule_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_marketingautomationrule_foreign_keys(foreign_keys):
    """Create foreign keys for MarketingAutomationRule table"""
    if not foreign_keys:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_marketingautomationrule_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '2' or args.full:
        ...
        if not create_marketingautomationrule_indexes(indexes):
            success = False
    
    if args.phase == '3' or args.full:
        print(f" Phase 3: Creating foreign keys for {TABLE_NAME}")
        if not create_marketingautomationrule_foreign_keys(foreign_keys):
            success = False
