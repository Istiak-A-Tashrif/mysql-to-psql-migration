#!/usr/bin/env python3
"""
Vendor Table Migration Script
=============================

This script provides a complete 3-phase migration approach specifically for the Vendor table:
...
- Handles Vendor-specific data types and constraints
- Manages foreign key dependencies (Company)
- Creates appropriate indexes for Vendor table
- Complex table with multiple contact fields

Usage: 
    python vendor_migration.py --phase=1
    python vendor_migration.py --phase=2
    python vendor_migration.py --phase=3
    python vendor_migration.py --full
    python vendor_migration.py --verify
"""
...
def get_vendor_table_info():
    """Get complete Vendor table information from MySQL including constraints"""
    ...
    indexes = extract_vendor_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_vendor_foreign_keys_from_ddl(mysql_ddl)
...
def extract_vendor_indexes_from_ddl(ddl):
    """Extract index definitions from Vendor table MySQL DDL"""
    indexes = []
...
def extract_vendor_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from Vendor table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to Vendor
...
def convert_vendor_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert Vendor table MySQL DDL to PostgreSQL DDL with Vendor-specific optimizations"""
    print(f" Converting Vendor table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    table_name = '"Vendor"' if preserve_case else 'vendor'
...
def create_vendor_indexes(indexes):
    """Create indexes for Vendor table"""
    if not indexes:
    ...
    for index in indexes:
        ...
        print(f" Creating Vendor index: {index_name}")
        
        sql_file = f"create_vendor_index_{index_name}.sql"
        ...
        if result and "CREATE INDEX" in str(result):
            print(f" Created Vendor index: {index_name}")
        else:
            print(f" Failed to create Vendor index {index_name}: {result}")
            success = False
    
    return success

def create_vendor_foreign_keys(foreign_keys):
    """Create foreign keys for Vendor table"""
    if not foreign_keys:
    ...
    for fk in foreign_keys:
        ...
        print(f" Creating Vendor FK: {constraint_name} -> {fk['ref_table']}")
        
        sql_file = f"create_vendor_fk_{constraint_name}.sql"
        with open(sql_file, "w", encoding="utf-8") as f:
            f.write(fk_sql)
        ...
        if result and "ALTER TABLE" in str(result):
            print(f" Created Vendor FK: {constraint_name}")
            created += 1
        else:
            print(f" Failed to create Vendor FK {constraint_name}: {result.stderr if result else 'Unknown error'}")
    
    print(f" Vendor Foreign Keys: {created} created")
    return True

def migrate_vendor_phase1():
    """Phase 1: Create Vendor table and import data"""
    print(f" Phase 1: Creating Vendor table and importing data")
    
    mysql_ddl, indexes, foreign_keys = get_vendor_table_info()
    if not mysql_ddl:
        return False
    
    postgres_ddl = convert_vendor_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    ...
    success = import_data_to_postgresql(TABLE_NAME, "Vendor data", preserve_case=PRESERVE_MYSQL_CASE, include_id=True)
    if not success:
        return False
...
def migrate_vendor_phase2():
    """Phase 2: Create indexes for Vendor table"""
    ...
    return create_vendor_indexes(indexes)

def migrate_vendor_phase3():
    """Phase 3: Create foreign keys for Vendor table"""
    ...
    return create_vendor_foreign_keys(foreign_keys)

def main():
    parser = argparse.ArgumentParser(description='Migrate Vendor table from MySQL to PostgreSQL')
    ...
    if args.full:
        ...
        success = (migrate_vendor_phase1() and 
                  migrate_vendor_phase2() and 
                  migrate_vendor_phase3())
    elif args.phase == 1:
        success = migrate_vendor_phase1()
    elif args.phase == 2:
        success = migrate_vendor_phase2()
    elif args.phase == 3:
        success = migrate_vendor_phase3()
    else:
        parser.print_help()
        return
