...
def get_labortag_table_info():
    """Get MySQL table information for LaborTag"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `LaborTag`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting LaborTag table structure: {result.stderr}")
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'LaborTag'"
        ])
...
')
        ...
        print(f"Error getting LaborTag table info: {e}")
        return None, None, None

def convert_labortag_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for LaborTag"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?LaborTag`?', f'CREATE TABLE "LaborTag"', postgres_ddl)
        ...
        print(f"Error converting LaborTag DDL: {e}")
        return None
...
def create_labortag_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating LaborTag table: {e}")
        return False
...
def create_labortag_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating LaborTag indexes: {e}")
        return False
...
def create_labortag_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating LaborTag foreign keys: {e}")
        return False
