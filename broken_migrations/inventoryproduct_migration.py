...
def get_inventoryproduct_table_info():
    """Get MySQL table information for InventoryProduct"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `InventoryProduct`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting InventoryProduct table structure: {result.stderr}")
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `InventoryProduct`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'InventoryProduct'"
        ])
        ...
        print(f"Error getting InventoryProduct table info: {e}")
        return None, None, None

def convert_inventoryproduct_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for InventoryProduct"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?InventoryProduct`?', f"CREATE TABLE "InventoryProduct"\', postgres_ddl)
        ...
        print(f"Error converting InventoryProduct DDL: {e}")
        return None
...
def create_inventoryproduct_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating InventoryProduct table: {e}")
        return False
...
def create_inventoryproduct_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating InventoryProduct indexes: {e}")
        return False
...
def create_inventoryproduct_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating InventoryProduct foreign keys: {e}")
        return False
