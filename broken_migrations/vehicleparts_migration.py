...
def get_vehicleparts_table_info():
    """Get MySQL table information for VehicleParts"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `VehicleParts`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting VehicleParts table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `VehicleParts`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'VehicleParts'"
        ])
        ...
        print(f"Error getting VehicleParts table info: {e}")
        return None, None, None

def convert_vehicleparts_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for VehicleParts"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?VehicleParts`?', f"CREATE TABLE "VehicleParts"\', postgres_ddl)
        ...
        print(f"Error converting VehicleParts DDL: {e}")
        return None
...
def create_vehicleparts_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating VehicleParts table: {e}")
        return False
...
def create_vehicleparts_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating VehicleParts indexes: {e}")
        return False
...
def create_vehicleparts_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating VehicleParts foreign keys: {e}")
        return False
