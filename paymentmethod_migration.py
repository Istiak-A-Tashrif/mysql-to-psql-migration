...
def get_paymentmethod_table_info():
    """Get MySQL table information for PaymentMethod"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `PaymentMethod`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting PaymentMethod table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW INDEX FROM `PaymentMethod`;'
        ])
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'PaymentMethod'"
        ])
...
')
        ...
        print(f"Error getting PaymentMethod table info: {e}")
        return None, None, None

def convert_paymentmethod_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for PaymentMethod"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?PaymentMethod`?', f'CREATE TABLE "PaymentMethod"', postgres_ddl)
        ...
        print(f"Error converting PaymentMethod DDL: {e}")
        return None
...
def create_paymentmethod_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating PaymentMethod table: {e}")
        return False
...
def create_paymentmethod_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating PaymentMethod indexes: {e}")
        return False
...
def create_paymentmethod_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating PaymentMethod foreign keys: {e}")
        return False
