...
def get_userfeedback_table_info():
    """Get MySQL table information for UserFeedback"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `UserFeedback`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting UserFeedback table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW INDEX FROM `UserFeedback`;'
        ])
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'UserFeedback'"
        ])
...
')
        ...
        print(f"Error getting UserFeedback table info: {e}")
        return None, None, None

def convert_userfeedback_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for UserFeedback"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?UserFeedback`?', f'CREATE TABLE "UserFeedback"', postgres_ddl)
        ...
        print(f"Error converting UserFeedback DDL: {e}")
        return None
...
def create_userfeedback_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating UserFeedback table: {e}")
        return False
...
def create_userfeedback_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating UserFeedback indexes: {e}")
        return False
...
def create_userfeedback_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating UserFeedback foreign keys: {e}")
        return False
