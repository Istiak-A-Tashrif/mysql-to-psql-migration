...
def get_taskuser_table_info():
    """Get MySQL table information for TaskUser"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `TaskUser`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting TaskUser table structure: {result.stderr}")
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'TaskUser'"
        ])
...
')
        ...
        print(f"Error getting TaskUser table info: {e}")
        return None, None, None

def convert_taskuser_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for TaskUser"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?TaskUser`?', f'CREATE TABLE "TaskUser"', postgres_ddl)
        ...
        print(f"Error converting TaskUser DDL: {e}")
        return None
...
def create_taskuser_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating TaskUser table: {e}")
        return False
...
def create_taskuser_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating TaskUser indexes: {e}")
        return False
...
def create_taskuser_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating TaskUser foreign keys: {e}")
        return False
