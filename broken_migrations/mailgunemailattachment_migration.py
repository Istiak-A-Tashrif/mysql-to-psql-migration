...
def get_mailgunemailattachment_table_info():
    """Get MySQL table information for MailgunEmailAttachment"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `MailgunEmailAttachment`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting MailgunEmailAttachment table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `MailgunEmailAttachment`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'MailgunEmailAttachment'"
        ])
        ...
        print(f"Error getting MailgunEmailAttachment table info: {e}")
        return None, None, None

def convert_mailgunemailattachment_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for MailgunEmailAttachment"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?MailgunEmailAttachment`?', f"CREATE TABLE "MailgunEmailAttachment"\', postgres_ddl)
        ...
        print(f"Error converting MailgunEmailAttachment DDL: {e}")
        return None
...
def create_mailgunemailattachment_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating MailgunEmailAttachment table: {e}")
        return False
...
def create_mailgunemailattachment_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating MailgunEmailAttachment indexes: {e}")
        return False
...
def create_mailgunemailattachment_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating MailgunEmailAttachment foreign keys: {e}")
        return False
