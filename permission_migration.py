...
def get_permission_table_info():
    """Get MySQL table information for Permission"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `Permission`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting Permission table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW INDEX FROM `Permission`;'
        ])
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'Permission'"
        ])
...
')
        ...
        print(f"Error getting Permission table info: {e}")
        return None, None, None

def convert_permission_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for Permission"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?Permission`?', f'CREATE TABLE "Permission"', postgres_ddl)
        ...
        print(f"Error converting Permission DDL: {e}")
        return None
...
def create_permission_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating Permission table: {e}")
        return False
...
def create_permission_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating Permission indexes: {e}")
        return False
...
def create_permission_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating Permission foreign keys: {e}")
        return False
