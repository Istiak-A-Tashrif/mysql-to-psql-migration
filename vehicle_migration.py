#!/usr/bin/env python3
"""
Vehicle Table Migration Script
=============================

This script provides a complete 3-phase migration approach specifically for the Vehicle table:
...
- Handles Vehicle-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for Vehicle table
- Large table with customer/vehicle data

Usage: 
    python vehicle_migration.py --phase=1
    python vehicle_migration.py --phase=2
    python vehicle_migration.py --phase=3
    python vehicle_migration.py --full
    python vehicle_migration.py --verify
"""
...
def get_vehicle_table_info():
    """Get complete Vehicle table information from MySQL including constraints"""
    ...
    indexes = extract_vehicle_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_vehicle_foreign_keys_from_ddl(mysql_ddl)
...
def extract_vehicle_indexes_from_ddl(ddl):
    """Extract index definitions from Vehicle table MySQL DDL"""
    indexes = []
...
def extract_vehicle_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from Vehicle table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to Vehicle - handle multi-word actions like "SET NULL"
...
def convert_vehicle_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert Vehicle table MySQL DDL to PostgreSQL DDL with Vehicle-specific optimizations"""
    print(f" Converting Vehicle table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_vehicle_column_definition(line, preserve_case)
            if processed_line:
...
def process_vehicle_column_definition(line, preserve_case):
    """Process a single column definition for Vehicle table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for Vehicle
    conversions = [
...
def create_vehicle_table(mysql_ddl):
    """Create Vehicle table in PostgreSQL"""
    postgres_ddl = convert_vehicle_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_vehicle_indexes(indexes):
...
def create_vehicle_foreign_keys(foreign_keys):
    """Create foreign keys for Vehicle table"""
    if not foreign_keys:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_vehicle_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '1' or args.full:
        ...
        if not create_vehicle_table(mysql_ddl):
            success = False
        else:
    ...
    if args.phase == '2' or args.full:
        ...
        if not create_vehicle_indexes(indexes):
            success = False
    
    if args.phase == '3' or args.full:
        print(f" Phase 3: Creating foreign keys for {TABLE_NAME}")
        if not create_vehicle_foreign_keys(foreign_keys):
            success = False
