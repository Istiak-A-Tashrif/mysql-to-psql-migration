...
This script provides         #            postgres_ddl = re.sub(r'CREATE TABLE `?DepositPayment`?', 'CREATE TABLE "DepositPayment"', postgres_ddl)
            
            # Quote column names (basic approach)
            postgres_ddl = re.sub(r'`([^`]+)`', r'"\1"', postgres_ddl)dle case sensitivity
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?DepositPayment`?', 'CREATE TABLE "DepositPayment"', postgres_ddl)
            
            # Quote column names (basic approach)
            postgres_ddl = re.sub(r'`([^`]+)`', r'"\1"', postgres_ddl)ete 3-phase migration approach specifically for the DepositPayment table:
...
def get_depositpayment_table_info():
    """Get MySQL table information for DepositPayment"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `DepositPayment`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting DepositPayment table structure: {result.stderr}")
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `DepositPayment`;"
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'DepositPayment'"
        ])
        ...
        print(f"Error getting DepositPayment table info: {e}")
        return None, None, None

def convert_depositpayment_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for DepositPayment"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?DepositPayment`?', f"CREATE TABLE "DepositPayment"\', postgres_ddl)
        ...
        print(f"Error converting DepositPayment DDL: {e}")
        return None
...
def create_depositpayment_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating DepositPayment table: {e}")
        return False
...
def create_depositpayment_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating DepositPayment indexes: {e}")
        return False
...
def create_depositpayment_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating DepositPayment foreign keys: {e}")
        return False
