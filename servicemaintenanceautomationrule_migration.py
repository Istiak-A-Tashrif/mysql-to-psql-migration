#!/usr/bin/env python3
"""
ServiceMaintenanceAutomationRule Table Migration Script
=============================

This script provides a complete 3-phase migration approach specifically for the ServiceMaintenanceAutomationRule table:
...
- Handles ServiceMaintenanceAutomationRule-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for ServiceMaintenanceAutomationRule table
- Large table with customer/servicemaintenanceautomationrule data

Usage: 
    python servicemaintenanceautomationrule_migration.py --phase=1
    python servicemaintenanceautomationrule_migration.py --phase=2
    python servicemaintenanceautomationrule_migration.py --phase=3
    python servicemaintenanceautomationrule_migration.py --full
    python servicemaintenanceautomationrule_migration.py --verify
"""
...
TABLE_NAME = "ServiceMaintenanceAutomationRule"

def get_servicemaintenanceautomationrule_table_info():
    """Get complete ServiceMaintenanceAutomationRule table information from MySQL including constraints"""
    ...
    indexes = extract_servicemaintenanceautomationrule_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_servicemaintenanceautomationrule_foreign_keys_from_ddl(mysql_ddl)
...
def extract_servicemaintenanceautomationrule_indexes_from_ddl(ddl):
    """Extract index definitions from ServiceMaintenanceAutomationRule table MySQL DDL"""
    indexes = []
    ...
    for match in matches:
        ...
        indexes.append({
            ...
            'table': 'ServiceMaintenanceAutomationRule'
        })
    
    return indexes

def extract_servicemaintenanceautomationrule_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from ServiceMaintenanceAutomationRule table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to ServiceMaintenanceAutomationRule - handle multi-word actions like "SET NULL"
...
def convert_servicemaintenanceautomationrule_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert ServiceMaintenanceAutomationRule table MySQL DDL to PostgreSQL DDL with ServiceMaintenanceAutomationRule-specific optimizations"""
    print(f" Converting ServiceMaintenanceAutomationRule table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_servicemaintenanceautomationrule_column_definition(line, preserve_case)
            if processed_line:
...
def process_servicemaintenanceautomationrule_column_definition(line, preserve_case):
    """Process a single column definition for ServiceMaintenanceAutomationRule table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for ServiceMaintenanceAutomationRule
    conversions = [
    ...
    # Handle specific ServiceMaintenanceAutomationRule issues: make first_name nullable to handle empty strings
...
def create_servicemaintenanceautomationrule_table(mysql_ddl):
    """Create ServiceMaintenanceAutomationRule table in PostgreSQL"""
    postgres_ddl = convert_servicemaintenanceautomationrule_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_servicemaintenanceautomationrule_indexes(indexes):
    """Create indexes for ServiceMaintenanceAutomationRule table"""
    if not indexes:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_servicemaintenanceautomationrule_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '1' or args.full:
        ...
        if not create_servicemaintenanceautomationrule_table(mysql_ddl):
            success = False
        else:
    ...
    if args.phase == '2' or args.full:
        ...
        if not create_servicemaintenanceautomationrule_indexes(indexes):
            success = False
    
    if args.phase == '3' or args.full:
        print(f" Phase 3: Creating foreign keys for {TABLE_NAME}")
        if not create_servicemaintenanceautomationrule_foreign_keys(foreign_keys):
            success = False
