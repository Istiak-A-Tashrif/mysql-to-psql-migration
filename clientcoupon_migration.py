#!/usr/bin/env python3
"""
ClientCoupon Table Migration Script
==================================

This script provides a complete 3-phase migration approach specifically for the ClientCoupon table:
...
- Handles ClientCoupon-specific data types and constraints
- Manages foreign key dependencies (Client, Coupon)
- Creates appropriate indexes for ClientCoupon table
- Handles datetime types and auto-increment

Usage: 
    python clientcoupon_migration.py --phase=1
    python clientcoupon_migration.py --phase=2
    python clientcoupon_migration.py --phase=3
    python clientcoupon_migration.py --full
    python clientcoupon_migration.py --verify
"""
...
TABLE_NAME = "ClientCoupon"

def get_clientcoupon_table_info():
    """Get complete ClientCoupon table information from MySQL including constraints"""
...
def extract_clientcoupon_foreign_keys_from_ddl(ddl):
    foreign_keys = []
...
def convert_clientcoupon_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    print(f"🔄 Converting ClientCoupon table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_clientcoupon_column_definition(line, preserve_case)
            if processed_line:
...
def process_clientcoupon_column_definition(line, preserve_case):
...
def create_clientcoupon_table(mysql_ddl):
...
def phase1_create_table_and_data():
    ...
    if not mysql_ddl:
        return False
    if not create_clientcoupon_table(mysql_ddl):
        return False
    if not export_and_clean_mysql_data(TABLE_NAME):
        return False
    if not import_data_to_postgresql(TABLE_NAME, "ClientCoupon", PRESERVE_MYSQL_CASE, include_id=True):
        return False
...
def phase3_create_foreign_keys():
    ...
    if mysql_ddl is None:
        return False
    return create_clientcoupon_foreign_keys(foreign_keys)
