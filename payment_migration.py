...
This script provides a complete 3-phase migration approach specifically for the Payment table:
...
- Handles Payment-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for Payment table
- Large table with customer/payment data

Usage: 
    python payment_migration.py --phase=1
    python payment_migration.py --phase=2
    python payment_migration.py --phase=3
    python payment_migration.py --full
    python payment_migration.py --verify
"""
...
def get_payment_table_info():
    """Get complete Payment table information from MySQL including constraints"""
    ...
    indexes = extract_payment_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_payment_foreign_keys_from_ddl(mysql_ddl)
...
def extract_payment_indexes_from_ddl(ddl):
    """Extract index definitions from Payment table MySQL DDL"""
    indexes = []
...
def extract_payment_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from Payment table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to Payment - handle multi-word actions like "SET NULL"
...
def convert_payment_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert Payment table MySQL DDL to PostgreSQL DDL with Payment-specific optimizations"""
    print(f"🔄 Converting Payment table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_payment_column_definition(line, preserve_case)
            if processed_line:
...
def process_payment_column_definition(line, preserve_case):
    """Process a single column definition for Payment table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for Payment
    conversions = [
        ...
        # Payment-specific ENUM handling
        (r"enum\('CARD','CHECK','CASH','OTHER','DEPOSIT'\)", 'payment_type_enum'),
    ]
...
def create_payment_table(mysql_ddl):
    """Create Payment table in PostgreSQL"""
    postgres_ddl = convert_payment_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_payment_indexes(indexes):
    ...
    for index in indexes:
        ...
        # Fix specific Payment table column name issues
        if 'date' in columns.lower():
            columns = columns.replace('"date"', '"DATE"').replace('date', '"DATE"')
...
def create_payment_foreign_keys(foreign_keys):
    """Create foreign keys for Payment table"""
    if not foreign_keys:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_payment_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '1' or args.full:
        ...
        if not create_payment_table(mysql_ddl):
            success = False
        else:
    ...
    if args.phase == '2' or args.full:
        ...
        if not create_payment_indexes(indexes):
            success = False
    
    if args.phase == '3' or args.full:
        print(f"🔗 Phase 3: Creating foreign keys for {TABLE_NAME}")
        if not create_payment_foreign_keys(foreign_keys):
            success = False
