#!/usr/bin/env python3
"""
ClientSmsAttachments Table Migration Script
==========================================

This script provides a complete 3-phase migration approach specifically for the ClientSmsAttachments table:
...
- Handles ClientSmsAttachments-specific data types and constraints
- Manages foreign key dependencies (ClientSMS)
- Creates appropriate indexes for ClientSmsAttachments table
- Handles datetime types and auto-increment

Usage: 
    python clientsmsattachments_migration.py --phase=1
    python clientsmsattachments_migration.py --phase=2
    python clientsmsattachments_migration.py --phase=3
    python clientsmsattachments_migration.py --full
    python clientsmsattachments_migration.py --verify
"""
...
TABLE_NAME = "ClientSmsAttachments"

def get_clientsmsattachments_table_info():
    """Get complete ClientSmsAttachments table information from MySQL including constraints"""
    ...
    indexes = extract_clientsmsattachments_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_clientsmsattachments_foreign_keys_from_ddl(mysql_ddl)
...
def extract_clientsmsattachments_indexes_from_ddl(ddl):
    indexes = []
    ...
    for match in matches:
        ...
        indexes.append({
            ...
            'table': 'ClientSmsAttachments'
        })
    return indexes

def extract_clientsmsattachments_foreign_keys_from_ddl(ddl):
    foreign_keys = []
...
def convert_clientsmsattachments_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    print(f"🔄 Converting ClientSmsAttachments table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_clientsmsattachments_column_definition(line, preserve_case)
            if processed_line:
...
def process_clientsmsattachments_column_definition(line, preserve_case):
...
def create_clientsmsattachments_table(mysql_ddl):
    postgres_ddl = convert_clientsmsattachments_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_clientsmsattachments_indexes(indexes):
    if not indexes:
...
def create_clientsmsattachments_foreign_keys(foreign_keys):
    if not foreign_keys:
...
def phase1_create_table_and_data():
    ...
    if not mysql_ddl:
        return False
    if not create_clientsmsattachments_table(mysql_ddl):
        return False
    if not export_and_clean_mysql_data(TABLE_NAME):
        return False
    if not import_data_to_postgresql(TABLE_NAME, "ClientSmsAttachments", PRESERVE_MYSQL_CASE, include_id=True):
        return False
...
def phase2_create_indexes():
    ...
    if mysql_ddl is None:
        return False
    return create_clientsmsattachments_indexes(indexes)
...
def phase3_create_foreign_keys():
    ...
    if mysql_ddl is None:
        return False
    return create_clientsmsattachments_foreign_keys(foreign_keys)
