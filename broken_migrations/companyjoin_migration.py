#!/usr/bin/env python3
"""
CompanyJoin Table Mig            '-e', f"SHOW CREATE TABLE `CompanyJoin`;""ation Script
==============              #            postgres_ddl = re.sub(r'CREATE TABLE `?CompanyJoin`?', 'CREATE TABLE "CompanyJoin"', postgres_ddl)
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?CompanyJoin`?', f'CREATE TABLE "CompanyJoin"', postgres_ddl)
...
def get_companyjoin_table_info():
    """Get MySQL table information for CompanyJoin"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `CompanyJoin`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting CompanyJoin table structure: {result.stderr}")
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `CompanyJoin`;"
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'CompanyJoin'"
        ])
        ...
        print(f"Error getting CompanyJoin table info: {e}")
        return None, None, None

def convert_companyjoin_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for CompanyJoin"""
    try:
        # Basic conversions
        ...
        print(f"Error converting CompanyJoin DDL: {e}")
        return None
...
def create_companyjoin_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating CompanyJoin table: {e}")
        return False
...
def create_companyjoin_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating CompanyJoin indexes: {e}")
        return False
...
def create_companyjoin_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating CompanyJoin foreign keys: {e}")
        return False
