...
def get_otherpayment_table_info():
    """Get MySQL table information for OtherPayment"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `OtherPayment`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting OtherPayment table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW INDEX FROM `OtherPayment`;'
        ])
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'OtherPayment'"
        ])
...
')
        ...
        print(f"Error getting OtherPayment table info: {e}")
        return None, None, None

def convert_otherpayment_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for OtherPayment"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?OtherPayment`?', f'CREATE TABLE "OtherPayment"', postgres_ddl)
        ...
        print(f"Error converting OtherPayment DDL: {e}")
        return None
...
def create_otherpayment_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating OtherPayment table: {e}")
        return False
...
def create_otherpayment_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating OtherPayment indexes: {e}")
        return False
...
def create_otherpayment_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating OtherPayment foreign keys: {e}")
        return False
