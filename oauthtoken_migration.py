#!/usr/bin/env python3
"""
OauthToken Table Migration Script
=============================

This script provides a complete 3-phase migration approach specifically for the OauthToken table:
...
- Handles OauthToken-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for OauthToken table
- Large table with customer/oauthtoken data

Usage: 
    python oauthtoken_migration.py --phase=1
    python oauthtoken_migration.py --phase=2
    python oauthtoken_migration.py --phase=3
    python oauthtoken_migration.py --full
    python oauthtoken_migration.py --verify
"""
...
def get_oauthtoken_table_info():
    ...
    indexes = extract_oauthtoken_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_oauthtoken_foreign_keys_from_ddl(mysql_ddl)
...
def extract_oauthtoken_indexes_from_ddl(ddl):
    """Extract index definitions from OauthToken table MySQL DDL"""
    indexes = []
...
def extract_oauthtoken_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from OauthToken table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to OauthToken - handle multi-word actions like "SET NULL"
...
def convert_oauthtoken_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert OauthToken table MySQL DDL to PostgreSQL DDL with OauthToken-specific optimizations"""
    print(f" Converting OauthToken table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_oauthtoken_column_definition(line, preserve_case)
            if processed_line:
...
def process_oauthtoken_column_definition(line, preserve_case):
    """Process a single column definition for OauthToken table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for OauthToken
    conversions = [
    ...
    # Handle specific OauthToken issues: make first_name nullable to handle empty strings
...
def create_oauthtoken_table(mysql_ddl):
    """Create OauthToken table in PostgreSQL"""
    postgres_ddl = convert_oauthtoken_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_oauthtoken_indexes(indexes):
    """Create indexes for OauthToken table"""
    if not indexes:
...
def create_oauthtoken_foreign_keys(foreign_keys):
    """Create foreign keys for OauthToken table"""
    if not foreign_keys:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_oauthtoken_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '1' or args.full:
        ...
        if not create_oauthtoken_table(mysql_ddl):
            success = False
        else:
    ...
    if args.phase == '2' or args.full:
        ...
        if not create_oauthtoken_indexes(indexes):
            success = False
    
    if args.phase == '3' or args.full:
        print(f" Phase 3: Creating foreign keys for {TABLE_NAME}")
        if not create_oauthtoken_foreign_keys(foreign_keys):
            success = False
