#!/usr/bin/env python3
"""
Category Table Migration Script
===============================

This script provides a complete 3-phase migration approach specifically for the Category table:
...
- Handles Category-specific data types and constraints
- Manages foreign key dependencies (Company)
- Creates appropriate indexes for Category table
- Simple table structure with standard fields

Usage: 
    python category_migration.py --phase=1
    python category_migration.py --phase=2
    python category_migration.py --phase=3
    python category_migration.py --full
    python category_migration.py --verify
"""
...
def get_category_table_info():
    """Get complete Category table information from MySQL including constraints"""
    ...
    indexes = extract_category_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_category_foreign_keys_from_ddl(mysql_ddl)
...
def extract_category_indexes_from_ddl(ddl):
    """Extract index definitions from Category table MySQL DDL"""
    indexes = []
...
def extract_category_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from Category table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to Category
...
def convert_category_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert Category table MySQL DDL to PostgreSQL DDL with Category-specific optimizations"""
    print(f"🔄 Converting Category table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    table_name = '"Category"' if preserve_case else 'category'
...
def create_category_indexes(indexes):
    ...
    for index in indexes:
        ...
        print(f"🔧 Creating Category index: {index_name}")
        
        # Write to file and execute
        sql_file = f"create_category_index_{index_name}.sql"
        ...
        if result and "CREATE INDEX" in str(result):
            print(f"✅ Created Category index: {index_name}")
        else:
            print(f"❌ Failed to create Category index {index_name}: {result}")
            success = False
    
    return success

def create_category_foreign_keys(foreign_keys):
    """Create foreign keys for Category table"""
    if not foreign_keys:
    ...
    for fk in foreign_keys:
        ...
        print(f"🔧 Creating Category FK: {constraint_name} -> {fk['ref_table']}")
        
        # Write to file and execute
        sql_file = f"create_category_fk_{constraint_name}.sql"
        with open(sql_file, "w", encoding="utf-8") as f:
            f.write(fk_sql)
        ...
        if result and "ALTER TABLE" in str(result):
            print(f"✅ Created Category FK: {constraint_name}")
            created += 1
        else:
            print(f"❌ Failed to create Category FK {constraint_name}: {result.stderr if result else 'Unknown error'}")
    
    print(f"🎯 Category Foreign Keys: {created} created, {skipped} skipped")
    return True

def migrate_category_phase1():
    """Phase 1: Create Category table and import data"""
    print(f"🚀 Phase 1: Creating Category table and importing data")
    
    mysql_ddl, indexes, foreign_keys = get_category_table_info()
    if not mysql_ddl:
        return False
    
    postgres_ddl = convert_category_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    ...
    success = import_data_to_postgresql(TABLE_NAME, "Category data", preserve_case=PRESERVE_MYSQL_CASE, include_id=True)
    if not success:
        return False
...
def migrate_category_phase2():
    """Phase 2: Create indexes for Category table"""
    ...
    return create_category_indexes(indexes)

def migrate_category_phase3():
    """Phase 3: Create foreign keys for Category table"""
    ...
    return create_category_foreign_keys(foreign_keys)

def main():
    parser = argparse.ArgumentParser(description='Migrate Category table from MySQL to PostgreSQL')
    ...
    if args.full:
        ...
        success = (migrate_category_phase1() and 
                  migrate_category_phase2() and 
                  migrate_category_phase3())
    elif args.phase == 1:
        success = migrate_category_phase1()
    elif args.phase == 2:
        success = migrate_category_phase2()
    elif args.phase == 3:
        success = migrate_category_phase3()
    else:
        parser.print_help()
        return
