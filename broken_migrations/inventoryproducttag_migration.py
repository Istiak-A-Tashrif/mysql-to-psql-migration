...
def get_inventoryproducttag_table_info():
    """Get MySQL table information for InventoryProductTag"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `InventoryProductTag`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting InventoryProductTag table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `InventoryProductTag`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'InventoryProductTag'"
        ])
        ...
        print(f"Error getting InventoryProductTag table info: {e}")
        return None, None, None

def convert_inventoryproducttag_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for InventoryProductTag"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?InventoryProductTag`?', f"CREATE TABLE "InventoryProductTag"\', postgres_ddl)
        ...
        print(f"Error converting InventoryProductTag DDL: {e}")
        return None
...
def create_inventoryproducttag_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating InventoryProductTag table: {e}")
        return False
...
def create_inventoryproducttag_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating InventoryProductTag indexes: {e}")
        return False
...
def create_inventoryproducttag_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating InventoryProductTag foreign keys: {e}")
        return False
