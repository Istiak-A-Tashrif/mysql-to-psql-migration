...
def get_inventoryproducthistory_table_info():
    """Get MySQL table information for InventoryProductHistory"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `InventoryProductHistory`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting InventoryProductHistory table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `InventoryProductHistory`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'InventoryProductHistory'"
        ])
        ...
        print(f"Error getting InventoryProductHistory table info: {e}")
        return None, None, None

def convert_inventoryproducthistory_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for InventoryProductHistory"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?InventoryProductHistory`?', f"CREATE TABLE "InventoryProductHistory"\', postgres_ddl)
        ...
        print(f"Error converting InventoryProductHistory DDL: {e}")
        return None
...
def create_inventoryproducthistory_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating InventoryProductHistory table: {e}")
        return False
...
def create_inventoryproducthistory_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating InventoryProductHistory indexes: {e}")
        return False
...
def create_inventoryproducthistory_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating InventoryProductHistory foreign keys: {e}")
        return False
