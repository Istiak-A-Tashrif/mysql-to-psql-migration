#!/usr/bin/env python3
"""
InvoiceInspection Table Migration Script
========================================

This script provides a complete 3-phase migration approach specifically for the InvoiceInspection table:
...
- Handles InvoiceInspection-specific data types and constraints
- Manages foreign key dependencies (Invoice)
- Creates appropriate indexes for InvoiceInspection table
- Large table with 6,184+ inspection records
- Converts tinyint(1) to BOOLEAN for driver/passenger flags

Usage: 
    python invoiceinspection_migration.py --phase=1
    python invoiceinspection_migration.py --phase=2
    python invoiceinspection_migration.py --phase=3
    python invoiceinspection_migration.py --full
    python invoiceinspection_migration.py --verify
"""
...
def extract_invoiceinspection_indexes_from_ddl(ddl):
    """Extract index definitions from InvoiceInspection table MySQL DDL"""
    indexes = []
    ...
    for match in matches:
        ...
        indexes.append({
            ...
            'table': 'InvoiceInspection'
        })
    
    return indexes

def extract_invoiceinspection_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from InvoiceInspection table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to InvoiceInspection - handle multi-word actions like "SET NULL"
...
def convert_invoiceinspection_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_invoiceinspection_column_definition(line, preserve_case)
            if processed_line:
...
def process_invoiceinspection_column_definition(line, preserve_case):
    """Process a single column definition for InvoiceInspection table"""
...
def create_invoiceinspection_table(mysql_ddl):
    """Create InvoiceInspection table in PostgreSQL"""
    postgres_ddl = convert_invoiceinspection_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_invoiceinspection_table_info()
    if not mysql_ddl:
        return
