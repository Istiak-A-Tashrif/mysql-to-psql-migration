...
def get_requestestimate_table_info():
    """Get MySQL table information for RequestEstimate"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `RequestEstimate`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting RequestEstimate table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `RequestEstimate`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'RequestEstimate'"
        ])
        ...
        print(f"Error getting RequestEstimate table info: {e}")
        return None, None, None

def convert_requestestimate_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for RequestEstimate"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?RequestEstimate`?', f"CREATE TABLE "RequestEstimate"\', postgres_ddl)
        ...
        print(f"Error converting RequestEstimate DDL: {e}")
        return None
...
def create_requestestimate_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating RequestEstimate table: {e}")
        return False
...
def create_requestestimate_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating RequestEstimate indexes: {e}")
        return False
...
def create_requestestimate_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating RequestEstimate foreign keys: {e}")
        return False
