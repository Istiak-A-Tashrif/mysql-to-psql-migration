...
def get_mailguncredential_table_info():
    """Get MySQL table information for MailgunCredential"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `MailgunCredential`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting MailgunCredential table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `MailgunCredential`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'MailgunCredential'"
        ])
        ...
        print(f"Error getting MailgunCredential table info: {e}")
        return None, None, None

def convert_mailguncredential_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for MailgunCredential"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?MailgunCredential`?', f"CREATE TABLE "MailgunCredential"\', postgres_ddl)
        ...
        print(f"Error converting MailgunCredential DDL: {e}")
        return None
...
def create_mailguncredential_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating MailgunCredential table: {e}")
        return False
...
def create_mailguncredential_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating MailgunCredential indexes: {e}")
        return False
...
def create_mailguncredential_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating MailgunCredential foreign keys: {e}")
        return False
