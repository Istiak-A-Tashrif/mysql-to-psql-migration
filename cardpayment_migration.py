#!/usr/bin/env python3
"""
CardPayment Table Migration Script
=================================

This script provides a complete 3-phase migration approach specifically for the CardPayment table:
...
- Handles CardPayment-specific data types and constraints
- Manages foreign key dependencies (Payment)
- Creates appropriate indexes for CardPayment table
- Handles enum types for card types

Usage: 
    python cardpayment_migration.py --phase=1
    python cardpayment_migration.py --phase=2
    python cardpayment_migration.py --phase=3
    python cardpayment_migration.py --full
    python cardpayment_migration.py --verify
"""
...
TABLE_NAME = "CardPayment"

def get_cardpayment_table_info():
    """Get complete CardPayment table information from MySQL including constraints"""
    ...
    indexes = extract_cardpayment_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_cardpayment_foreign_keys_from_ddl(mysql_ddl)
...
def extract_cardpayment_indexes_from_ddl(ddl):
    indexes = []
...
def extract_cardpayment_foreign_keys_from_ddl(ddl):
    foreign_keys = []
...
def convert_cardpayment_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    print(f"🔄 Converting CardPayment table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_cardpayment_column_definition(line, preserve_case)
            if processed_line:
...
def process_cardpayment_column_definition(line, preserve_case):
    ...
    if enum_match:
        ...
        print(f"🔧 Converted ENUM to VARCHAR for CardPayment")
...
def create_cardpayment_table(mysql_ddl):
    postgres_ddl = convert_cardpayment_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_cardpayment_indexes(indexes):
    if not indexes:
...
def create_cardpayment_foreign_keys(foreign_keys):
    if not foreign_keys:
...
def phase1_create_table_and_data():
    ...
    mysql_ddl, indexes, foreign_keys = get_cardpayment_table_info()
    if not mysql_ddl:
        return False
    if not create_cardpayment_table(mysql_ddl):
        return False
    if not export_and_clean_mysql_data(TABLE_NAME):
        return False
    if not import_data_to_postgresql(TABLE_NAME, "CardPayment", PRESERVE_MYSQL_CASE, include_id=True):
        return False
...
def phase2_create_indexes():
    ...
    if mysql_ddl is None:
        return False
    return create_cardpayment_indexes(indexes)
...
def phase3_create_foreign_keys():
    ...
    if mysql_ddl is None:
        return False
    return create_cardpayment_foreign_keys(foreign_keys)
