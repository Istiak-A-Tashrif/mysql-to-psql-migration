#!/usr/bin/env python3
"""
Status Table Migration Script
=============================

This script provides a complete 3-phase migration approach specifically for the Status table:
...
- Handles Status-specific data types and constraints
- Manages foreign key dependencies (Company)
- Creates appropriate indexes for Status table
- Simple table structure with color fields

Usage: 
    python status_migration.py --phase=1
    python status_migration.py --phase=2
    python status_migration.py --phase=3
    python status_migration.py --full
    python status_migration.py --verify
"""
...
def get_status_table_info():
    """Get complete Status table information from MySQL including constraints"""
    ...
    indexes = extract_status_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_status_foreign_keys_from_ddl(mysql_ddl)
...
def extract_status_indexes_from_ddl(ddl):
    """Extract index definitions from Status table MySQL DDL"""
    indexes = []
...
def extract_status_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from Status table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to Status
...
def convert_status_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert Status table MySQL DDL to PostgreSQL DDL with Status-specific optimizations"""
    print(f"🔄 Converting Status table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    table_name = '"Status"' if preserve_case else 'status'
...
def create_status_indexes(indexes):
    ...
    for index in indexes:
        ...
        print(f"🔧 Creating Status index: {index_name}")
        
        sql_file = f"create_status_index_{index_name}.sql"
        ...
        if result and "CREATE INDEX" in str(result):
            print(f"✅ Created Status index: {index_name}")
        else:
            print(f"❌ Failed to create Status index {index_name}: {result}")
            success = False
    
    return success

def create_status_foreign_keys(foreign_keys):
    """Create foreign keys for Status table"""
    if not foreign_keys:
    ...
    for fk in foreign_keys:
        ...
        print(f"🔧 Creating Status FK: {constraint_name} -> {fk['ref_table']}")
        
        sql_file = f"create_status_fk_{constraint_name}.sql"
        with open(sql_file, "w", encoding="utf-8") as f:
            f.write(fk_sql)
        ...
        if result and "ALTER TABLE" in str(result):
            print(f"✅ Created Status FK: {constraint_name}")
            created += 1
        else:
            print(f"❌ Failed to create Status FK {constraint_name}: {result.stderr if result else 'Unknown error'}")
    
    print(f"🎯 Status Foreign Keys: {created} created")
    return True

def migrate_status_phase1():
    """Phase 1: Create Status table and import data"""
    print(f"🚀 Phase 1: Creating Status table and importing data")
    
    mysql_ddl, indexes, foreign_keys = get_status_table_info()
    if not mysql_ddl:
        return False
    
    postgres_ddl = convert_status_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    ...
    success = import_data_to_postgresql(TABLE_NAME, "Status data", preserve_case=PRESERVE_MYSQL_CASE, include_id=True)
    if not success:
        return False
...
def migrate_status_phase2():
    """Phase 2: Create indexes for Status table"""
    ...
    return create_status_indexes(indexes)

def migrate_status_phase3():
    """Phase 3: Create foreign keys for Status table"""
    ...
    return create_status_foreign_keys(foreign_keys)

def main():
    parser = argparse.ArgumentParser(description='Migrate Status table from MySQL to PostgreSQL')
    ...
    if args.full:
        ...
        success = (migrate_status_phase1() and 
                  migrate_status_phase2() and 
                  migrate_status_phase3())
    elif args.phase == 1:
        success = migrate_status_phase1()
    elif args.phase == 2:
        success = migrate_status_phase2()
    elif args.phase == 3:
        success = migrate_status_phase3()
    else:
        parser.print_help()
        return
