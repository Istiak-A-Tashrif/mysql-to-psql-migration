#!/usr/bin/env python3
"""
ClientCall Table Migration Script
================================

This script provides a complete 3-phase migration approach specifically for the ClientCall table:
...
- Handles ClientCall-specific data types and constraints
- Manages foreign key dependencies (Client, Company, User)
- Creates appropriate indexes for ClientCall table
- Handles enum types and reserved words (from, to)

Usage: 
    python clientcall_migration.py --phase=1
    python clientcall_migration.py --phase=2
    python clientcall_migration.py --phase=3
    python clientcall_migration.py --full
    python clientcall_migration.py --verify
"""
...
def get_clientcall_table_info():
    ...
    indexes = extract_clientcall_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_clientcall_foreign_keys_from_ddl(mysql_ddl)
...
def extract_clientcall_indexes_from_ddl(ddl):
    indexes = []
...
def extract_clientcall_foreign_keys_from_ddl(ddl):
    foreign_keys = []
...
def convert_clientcall_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    print(f"🔄 Converting ClientCall table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_clientcall_column_definition(line, preserve_case)
            if processed_line:
...
def process_clientcall_column_definition(line, preserve_case):
...
def create_clientcall_table(mysql_ddl):
    postgres_ddl = convert_clientcall_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_clientcall_indexes(indexes):
    if not indexes:
...
def create_clientcall_foreign_keys(foreign_keys):
    if not foreign_keys:
...
def phase1_create_table_and_data():
    ...
    if not mysql_ddl:
        return False
    if not create_clientcall_table(mysql_ddl):
        return False
    if not export_and_clean_mysql_data(TABLE_NAME):
        return False
    if not import_data_to_postgresql(TABLE_NAME, "ClientCall", PRESERVE_MYSQL_CASE, include_id=True):
        return False
...
def phase3_create_foreign_keys():
    ...
    if mysql_ddl is None:
        return False
    return create_clientcall_foreign_keys(foreign_keys)
