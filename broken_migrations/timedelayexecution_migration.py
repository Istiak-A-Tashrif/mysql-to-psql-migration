...
def get_timedelayexecution_table_info():
    """Get MySQL table information for TimeDelayExecution"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `TimeDelayExecution`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting TimeDelayExecution table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `TimeDelayExecution`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'TimeDelayExecution'"
        ])
        ...
        print(f"Error getting TimeDelayExecution table info: {e}")
        return None, None, None

def convert_timedelayexecution_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for TimeDelayExecution"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?TimeDelayExecution`?', f"CREATE TABLE "TimeDelayExecution"\', postgres_ddl)
        ...
        print(f"Error converting TimeDelayExecution DDL: {e}")
        return None
...
def create_timedelayexecution_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating TimeDelayExecution table: {e}")
        return False
...
def create_timedelayexecution_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating TimeDelayExecution indexes: {e}")
        return False
...
def create_timedelayexecution_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating TimeDelayExecution foreign keys: {e}")
        return False
