...
def get_servicemaintenancestage_table_info():
    """Get MySQL table information for ServiceMaintenanceStage"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `ServiceMaintenanceStage`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting ServiceMaintenanceStage table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `ServiceMaintenanceStage`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'ServiceMaintenanceStage'"
        ])
        ...
        print(f"Error getting ServiceMaintenanceStage table info: {e}")
        return None, None, None

def convert_servicemaintenancestage_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for ServiceMaintenanceStage"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?ServiceMaintenanceStage`?', f"CREATE TABLE "ServiceMaintenanceStage"\', postgres_ddl)
        ...
        print(f"Error converting ServiceMaintenanceStage DDL: {e}")
        return None
...
def create_servicemaintenancestage_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating ServiceMaintenanceStage table: {e}")
        return False
...
def create_servicemaintenancestage_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating ServiceMaintenanceStage indexes: {e}")
        return False
...
def create_servicemaintenancestage_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating ServiceMaintenanceStage foreign keys: {e}")
        return False
