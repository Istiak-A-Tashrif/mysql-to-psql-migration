...
This script provides a complete 3-phase migration approach specifically for the InventoryProductHistory table:
...
- Handles InventoryProductHistory-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for InventoryProductHistory table
- Large table with customer/inventoryproducthistory data

Usage: 
    python inventoryproducthistory_migration.py --phase=1
    python inventoryproducthistory_migration.py --phase=2
    python inventoryproducthistory_migration.py --phase=3
    python inventoryproducthistory_migration.py --full
    python inventoryproducthistory_migration.py --verify
"""
...
def get_inventoryproducthistory_table_info():
    """Get complete InventoryProductHistory table information from MySQL including constraints"""
    ...
    indexes = extract_inventoryproducthistory_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_inventoryproducthistory_foreign_keys_from_ddl(mysql_ddl)
...
def extract_inventoryproducthistory_indexes_from_ddl(ddl):
    """Extract index definitions from InventoryProductHistory table MySQL DDL"""
    indexes = []
    ...
    for match in matches:
        ...
        indexes.append({
            ...
            'table': 'InventoryProductHistory'
        })
    
    return indexes

def extract_inventoryproducthistory_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from InventoryProductHistory table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to InventoryProductHistory - handle multi-word actions like "SET NULL"
...
def convert_inventoryproducthistory_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert InventoryProductHistory table MySQL DDL to PostgreSQL DDL with InventoryProductHistory-specific optimizations"""
    print(f" Converting InventoryProductHistory table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_inventoryproducthistory_column_definition(line, preserve_case)
            if processed_line:
...
def process_inventoryproducthistory_column_definition(line, preserve_case):
    """Process a single column definition for InventoryProductHistory table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for InventoryProductHistory
    conversions = [
    ...
    # Handle specific InventoryProductHistory issues: make first_name nullable to handle empty strings
...
def create_inventoryproducthistory_table(mysql_ddl):
    """Create InventoryProductHistory table in PostgreSQL"""
    postgres_ddl = convert_inventoryproducthistory_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_inventoryproducthistory_indexes(indexes):
    """Create indexes for InventoryProductHistory table"""
    if not indexes:
...
def create_inventoryproducthistory_foreign_keys(foreign_keys):
    """Create foreign keys for InventoryProductHistory table"""
    if not foreign_keys:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_inventoryproducthistory_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '1' or args.full:
        ...
        if not create_inventoryproducthistory_table(mysql_ddl):
            success = False
        else:
    ...
    if args.phase == '2' or args.full:
        ...
        if not create_inventoryproducthistory_indexes(indexes):
            success = False
    
    if args.phase == '3' or args.full:
        print(f" Phase 3: Creating foreign keys for {TABLE_NAME}")
        if not create_inventoryproducthistory_foreign_keys(foreign_keys):
            success = False
