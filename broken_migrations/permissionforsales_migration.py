...
def get_permissionforsales_table_info():
    """Get MySQL table information for PermissionForSales"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `PermissionForSales`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting PermissionForSales table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `PermissionForSales`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'PermissionForSales'"
        ])
        ...
        print(f"Error getting PermissionForSales table info: {e}")
        return None, None, None

def convert_permissionforsales_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for PermissionForSales"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?PermissionForSales`?', f"CREATE TABLE "PermissionForSales"\', postgres_ddl)
        ...
        print(f"Error converting PermissionForSales DDL: {e}")
        return None
...
def create_permissionforsales_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating PermissionForSales table: {e}")
        return False
...
def create_permissionforsales_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating PermissionForSales indexes: {e}")
        return False
...
def create_permissionforsales_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating PermissionForSales foreign keys: {e}")
        return False
