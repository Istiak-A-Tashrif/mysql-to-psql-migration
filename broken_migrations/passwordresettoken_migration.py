...
def get_passwordresettoken_table_info():
    """Get MySQL table information for PasswordResetToken"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `PasswordResetToken`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting PasswordResetToken table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `PasswordResetToken`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'PasswordResetToken'"
        ])
        ...
        print(f"Error getting PasswordResetToken table info: {e}")
        return None, None, None

def convert_passwordresettoken_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for PasswordResetToken"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?PasswordResetToken`?', f"CREATE TABLE "PasswordResetToken"\', postgres_ddl)
        ...
        print(f"Error converting PasswordResetToken DDL: {e}")
        return None
...
def create_passwordresettoken_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating PasswordResetToken table: {e}")
        return False
...
def create_passwordresettoken_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating PasswordResetToken indexes: {e}")
        return False
...
def create_passwordresettoken_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating PasswordResetToken foreign keys: {e}")
        return False
