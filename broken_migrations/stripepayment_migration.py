...
def get_stripepayment_table_info():
    """Get MySQL table information for StripePayment"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `StripePayment`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting StripePayment table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `StripePayment`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'StripePayment'"
        ])
        ...
        print(f"Error getting StripePayment table info: {e}")
        return None, None, None

def convert_stripepayment_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for StripePayment"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?StripePayment`?', f"CREATE TABLE "StripePayment"\', postgres_ddl)
        ...
        print(f"Error converting StripePayment DDL: {e}")
        return None
...
def create_stripepayment_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating StripePayment table: {e}")
        return False
...
def create_stripepayment_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating StripePayment indexes: {e}")
        return False
...
def create_stripepayment_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating StripePayment foreign keys: {e}")
        return False
