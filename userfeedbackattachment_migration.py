...
def get_userfeedbackattachment_table_info():
    """Get MySQL table information for UserFeedbackAttachment"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `UserFeedbackAttachment`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting UserFeedbackAttachment table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW INDEX FROM `UserFeedbackAttachment`;'
        ])
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'UserFeedbackAttachment'"
        ])
...
')
        ...
        print(f"Error getting UserFeedbackAttachment table info: {e}")
        return None, None, None

def convert_userfeedbackattachment_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for UserFeedbackAttachment"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?UserFeedbackAttachment`?', f'CREATE TABLE "UserFeedbackAttachment"', postgres_ddl)
        ...
        print(f"Error converting UserFeedbackAttachment DDL: {e}")
        return None
...
def create_userfeedbackattachment_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating UserFeedbackAttachment table: {e}")
        return False
...
def create_userfeedbackattachment_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating UserFeedbackAttachment indexes: {e}")
        return False
...
def create_userfeedbackattachment_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating UserFeedbackAttachment foreign keys: {e}")
        return False
