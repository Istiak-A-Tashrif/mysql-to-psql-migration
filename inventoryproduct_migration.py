...
This script provides a complete 3-phase migration approach specifically for the InventoryProduct table:
...
- Handles InventoryProduct-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for InventoryProduct table
- Large table with customer/inventoryproduct data

Usage: 
    python inventoryproduct_migration.py --phase=1
    python inventoryproduct_migration.py --phase=2
    python inventoryproduct_migration.py --phase=3
    python inventoryproduct_migration.py --full
    python inventoryproduct_migration.py --verify
"""
...
def get_inventoryproduct_table_info():
    """Get complete InventoryProduct table information from MySQL including constraints"""
    ...
    indexes = extract_inventoryproduct_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_inventoryproduct_foreign_keys_from_ddl(mysql_ddl)
...
def extract_inventoryproduct_indexes_from_ddl(ddl):
    """Extract index definitions from InventoryProduct table MySQL DDL"""
    indexes = []
...
def extract_inventoryproduct_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from InventoryProduct table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to InventoryProduct - handle multi-word actions like "SET NULL"
...
def convert_inventoryproduct_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert InventoryProduct table MySQL DDL to PostgreSQL DDL with InventoryProduct-specific optimizations"""
    print(f" Converting InventoryProduct table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_inventoryproduct_column_definition(line, preserve_case)
            if processed_line:
...
def process_inventoryproduct_column_definition(line, preserve_case):
    """Process a single column definition for InventoryProduct table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for InventoryProduct
    conversions = [
    ...
    # Handle specific InventoryProduct issues: make first_name nullable to handle empty strings
...
def create_inventoryproduct_table(mysql_ddl):
    """Create InventoryProduct table in PostgreSQL"""
    postgres_ddl = convert_inventoryproduct_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_inventoryproduct_indexes(indexes):
    """Create indexes for InventoryProduct table"""
    if not indexes:
...
def create_inventoryproduct_foreign_keys(foreign_keys):
    """Create foreign keys for InventoryProduct table"""
    if not foreign_keys:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_inventoryproduct_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '1' or args.full:
        ...
        if not create_inventoryproduct_table(mysql_ddl):
            success = False
        else:
    ...
    if args.phase == '2' or args.full:
        ...
        if not create_inventoryproduct_indexes(indexes):
            success = False
    
    if args.phase == '3' or args.full:
        print(f" Phase 3: Creating foreign keys for {TABLE_NAME}")
        if not create_inventoryproduct_foreign_keys(foreign_keys):
            success = False
