...
def get_permissionfortechnician_table_info():
    """Get MySQL table information for PermissionForTechnician"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `PermissionForTechnician`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting PermissionForTechnician table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `PermissionForTechnician`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'PermissionForTechnician'"
        ])
        ...
        print(f"Error getting PermissionForTechnician table info: {e}")
        return None, None, None

def convert_permissionfortechnician_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for PermissionForTechnician"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?PermissionForTechnician`?', f"CREATE TABLE "PermissionForTechnician"\', postgres_ddl)
        ...
        print(f"Error converting PermissionForTechnician DDL: {e}")
        return None
...
def create_permissionfortechnician_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating PermissionForTechnician table: {e}")
        return False
...
def create_permissionfortechnician_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating PermissionForTechnician indexes: {e}")
        return False
...
def create_permissionfortechnician_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating PermissionForTechnician foreign keys: {e}")
        return False
