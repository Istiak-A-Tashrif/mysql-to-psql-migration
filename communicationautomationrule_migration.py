#!/usr/bin/env python3
"""
CommunicationAutomationRule Table Migration Script
=================================================

This script provides a complete 3-phase migration approach specifically for the CommunicationAutomationRule table:
...
- Handles CommunicationAutomationRule-specific data types and constraints
- Manages foreign key dependencies (Column)
- Creates appropriate indexes for CommunicationAutomationRule table
- Handles enum types and tinyint(1) to boolean conversion

Usage: 
    python communicationautomationrule_migration.py --phase=1
    python communicationautomationrule_migration.py --phase=2
    python communicationautomationrule_migration.py --phase=3
    python communicationautomationrule_migration.py --full
    python communicationautomationrule_migration.py --verify
"""
...
TABLE_NAME = "CommunicationAutomationRule"

def get_communicationautomationrule_table_info():
    """Get complete CommunicationAutomationRule table information from MySQL including constraints"""
    ...
    indexes = extract_communicationautomationrule_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_communicationautomationrule_foreign_keys_from_ddl(mysql_ddl)
...
def extract_communicationautomationrule_indexes_from_ddl(ddl):
    indexes = []
    ...
    for match in matches:
        ...
        indexes.append({
            ...
            'table': 'CommunicationAutomationRule'
        })
    return indexes

def extract_communicationautomationrule_foreign_keys_from_ddl(ddl):
    foreign_keys = []
...
def convert_communicationautomationrule_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    print(f"🔄 Converting CommunicationAutomationRule table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_communicationautomationrule_column_definition(line, preserve_case)
            if processed_line:
...
def process_communicationautomationrule_column_definition(line, preserve_case):
    ...
    if enum_match:
        ...
        print(f"🔧 Converted ENUM to VARCHAR for CommunicationAutomationRule")
    
    # Handle tinyint(1) to boolean conversion
    if 'tinyint(1)' in line:
        ...
        print(f"🔧 Converted tinyint(1) to BOOLEAN for CommunicationAutomationRule")
...
def create_communicationautomationrule_table(mysql_ddl):
    postgres_ddl = convert_communicationautomationrule_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_communicationautomationrule_indexes(indexes):
    if not indexes:
...
def create_communicationautomationrule_foreign_keys(foreign_keys):
    if not foreign_keys:
...
def phase1_create_table_and_data():
    ...
    mysql_ddl, indexes, foreign_keys = get_communicationautomationrule_table_info()
    if not mysql_ddl:
        return False
    if not create_communicationautomationrule_table(mysql_ddl):
        return False
...
def phase2_create_indexes():
    ...
    if mysql_ddl is None:
        return False
    return create_communicationautomationrule_indexes(indexes)
...
def phase3_create_foreign_keys():
    ...
    if mysql_ddl is None:
        return False
    return create_communicationautomationrule_foreign_keys(foreign_keys)
