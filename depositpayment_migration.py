#!/usr/bin/env python3
"""
DepositPayment Table Migration Script
=============================

This script provides a complete 3-phase migration approach specifically for the DepositPayment table:
...
- Handles DepositPayment-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for DepositPayment table
- Large table with customer/depositpayment data

Usage: 
    python depositpayment_migration.py --phase=1
    python depositpayment_migration.py --phase=2
    python depositpayment_migration.py --phase=3
    python depositpayment_migration.py --full
    python depositpayment_migration.py --verify
"""
...
TABLE_NAME = "DepositPayment"

def get_depositpayment_table_info():
    """Get complete DepositPayment table information from MySQL including constraints"""
    ...
    indexes = extract_depositpayment_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_depositpayment_foreign_keys_from_ddl(mysql_ddl)
...
def extract_depositpayment_indexes_from_ddl(ddl):
    """Extract index definitions from DepositPayment table MySQL DDL"""
    indexes = []
...
def extract_depositpayment_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from DepositPayment table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to DepositPayment - handle multi-word actions like "SET NULL"
...
def convert_depositpayment_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert DepositPayment table MySQL DDL to PostgreSQL DDL with DepositPayment-specific optimizations"""
    print(f" Converting DepositPayment table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_depositpayment_column_definition(line, preserve_case)
            if processed_line:
...
def process_depositpayment_column_definition(line, preserve_case):
    """Process a single column definition for DepositPayment table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for DepositPayment
    conversions = [
    ...
    # Handle specific DepositPayment issues: make first_name nullable to handle empty strings
...
def create_depositpayment_table(mysql_ddl):
    """Create DepositPayment table in PostgreSQL"""
    postgres_ddl = convert_depositpayment_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_depositpayment_indexes(indexes):
    """Create indexes for DepositPayment table"""
    if not indexes:
...
def create_depositpayment_foreign_keys(foreign_keys):
    """Create foreign keys for DepositPayment table"""
    if not foreign_keys:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_depositpayment_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '2' or args.full:
        ...
        if not create_depositpayment_indexes(indexes):
            success = False
    
    if args.phase == '3' or args.full:
        print(f" Phase 3: Creating foreign keys for {TABLE_NAME}")
        if not create_depositpayment_foreign_keys(foreign_keys):
            success = False
