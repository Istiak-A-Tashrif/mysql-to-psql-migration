...
def get_invoicetags_table_info():
    """Get MySQL table information for InvoiceTags"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `InvoiceTags`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting InvoiceTags table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `InvoiceTags`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'InvoiceTags'"
        ])
        ...
        print(f"Error getting InvoiceTags table info: {e}")
        return None, None, None

def convert_invoicetags_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for InvoiceTags"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?InvoiceTags`?', f"CREATE TABLE "InvoiceTags"\', postgres_ddl)
        ...
        print(f"Error converting InvoiceTags DDL: {e}")
        return None
...
def create_invoicetags_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating InvoiceTags table: {e}")
        return False
...
def create_invoicetags_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating InvoiceTags indexes: {e}")
        return False
...
def create_invoicetags_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating InvoiceTags foreign keys: {e}")
        return False
