#!/usr/bin/env python3
"""
Group Table Migration Script
=============================

This script provides a complete 3-phase migration approach specifically for the Group table:
...
- Handles Group-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for Group table
- Large table with customer/group data

Usage: 
    python group_migration.py --phase=1
    python group_migration.py --phase=2
    python group_migration.py --phase=3
    python group_migration.py --full
    python group_migration.py --verify
"""
...
def get_group_table_info():
    """Get complete Group table information from MySQL including constraints"""
    ...
    indexes = extract_group_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_group_foreign_keys_from_ddl(mysql_ddl)
...
def extract_group_indexes_from_ddl(ddl):
    """Extract index definitions from Group table MySQL DDL"""
    indexes = []
...
def extract_group_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from Group table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to Group - handle multi-word actions like "SET NULL"
...
def convert_group_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert Group table MySQL DDL to PostgreSQL DDL with Group-specific optimizations"""
    print(f" Converting Group table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_group_column_definition(line, preserve_case)
            if processed_line:
...
def process_group_column_definition(line, preserve_case):
    """Process a single column definition for Group table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for Group
    conversions = [
    ...
    # Handle specific Group issues: make first_name nullable to handle empty strings
...
def create_group_table(mysql_ddl):
    """Create Group table in PostgreSQL"""
    postgres_ddl = convert_group_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_group_indexes(indexes):
...
def create_group_foreign_keys(foreign_keys):
    """Create foreign keys for Group table"""
    if not foreign_keys:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_group_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '1' or args.full:
        ...
        if not create_group_table(mysql_ddl):
            success = False
        else:
    ...
    if args.phase == '2' or args.full:
        ...
        if not create_group_indexes(indexes):
            success = False
    
    if args.phase == '3' or args.full:
        print(f" Phase 3: Creating foreign keys for {TABLE_NAME}")
        if not create_group_foreign_keys(foreign_keys):
            success = False
