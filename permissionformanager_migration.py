...
def get_permissionformanager_table_info():
    """Get MySQL table information for PermissionForManager"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `PermissionForManager`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting PermissionForManager table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW INDEX FROM `PermissionForManager`;'
        ])
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'PermissionForManager'"
        ])
...
')
        ...
        print(f"Error getting PermissionForManager table info: {e}")
        return None, None, None

def convert_permissionformanager_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for PermissionForManager"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?PermissionForManager`?', f'CREATE TABLE "PermissionForManager"', postgres_ddl)
        ...
        print(f"Error converting PermissionForManager DDL: {e}")
        return None
...
def create_permissionformanager_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating PermissionForManager table: {e}")
        return False
...
def create_permissionformanager_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating PermissionForManager indexes: {e}")
        return False
...
def create_permissionformanager_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating PermissionForManager foreign keys: {e}")
        return False
