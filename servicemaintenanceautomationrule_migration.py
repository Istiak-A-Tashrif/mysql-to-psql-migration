...
def get_servicemaintenanceautomationrule_table_info():
    """Get MySQL table information for ServiceMaintenanceAutomationRule"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `ServiceMaintenanceAutomationRule`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting ServiceMaintenanceAutomationRule table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW INDEX FROM `ServiceMaintenanceAutomationRule`;'
        ])
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'ServiceMaintenanceAutomationRule'"
        ])
...
')
        ...
        print(f"Error getting ServiceMaintenanceAutomationRule table info: {e}")
        return None, None, None

def convert_servicemaintenanceautomationrule_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for ServiceMaintenanceAutomationRule"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?ServiceMaintenanceAutomationRule`?', f'CREATE TABLE "ServiceMaintenanceAutomationRule"', postgres_ddl)
        ...
        print(f"Error converting ServiceMaintenanceAutomationRule DDL: {e}")
        return None
...
def create_servicemaintenanceautomationrule_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating ServiceMaintenanceAutomationRule table: {e}")
        return False
...
def create_servicemaintenanceautomationrule_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating ServiceMaintenanceAutomationRule indexes: {e}")
        return False
...
def create_servicemaintenanceautomationrule_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating ServiceMaintenanceAutomationRule foreign keys: {e}")
        return False
