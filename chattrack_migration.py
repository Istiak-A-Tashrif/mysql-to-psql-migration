#!/usr/bin/env python3
"""
ChatTrack Table Migration Script
===============================

This script provides a complete 3-phase migration approach specifically for the ChatTrack table:
...
- Handles ChatTrack-specific data types and constraints
- Manages foreign key dependencies (Message)
- Creates appropriate indexes for ChatTrack table
- Handles enum types and boolean fields

Usage: 
    python chattrack_migration.py --phase=1
    python chattrack_migration.py --phase=2
    python chattrack_migration.py --phase=3
    python chattrack_migration.py --full
    python chattrack_migration.py --verify
"""
...
def extract_chattrack_foreign_keys_from_ddl(ddl):
    foreign_keys = []
...
def convert_chattrack_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_chattrack_column_definition(line, preserve_case)
            if processed_line:
...
def process_chattrack_column_definition(line, preserve_case):
...
def create_chattrack_table(mysql_ddl):
...
def phase1_create_table_and_data():
    ...
    if not mysql_ddl:
        return False
    if not create_chattrack_table(mysql_ddl):
        return False
    if not export_and_clean_mysql_data(TABLE_NAME):
        return False
    if not import_data_to_postgresql(TABLE_NAME, "ChatTrack", PRESERVE_MYSQL_CASE, include_id=True):
        return False
...
def phase3_create_foreign_keys():
    ...
    if mysql_ddl is None:
        return False
    return create_chattrack_foreign_keys(foreign_keys)
