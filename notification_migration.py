#!/usr/bin/env python3
"""
Notification Table Migration Script
===================================

This script provides a complete 3-phase migration approach specifically for the Notification table:
...
- Handles Notification-specific data types and constraints
- Manages foreign key dependencies (User, Company)
- Creates appropriate indexes for Notification table
- Large table with 20,892+ notification records

Usage: 
    python notification_migration.py --phase=1
    python notification_migration.py --phase=2
    python notification_migration.py --phase=3
    python notification_migration.py --full
    python notification_migration.py --verify
"""
...
def get_notification_table_info():
    """Get complete Notification table information from MySQL including constraints"""
    ...
    indexes = extract_notification_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_notification_foreign_keys_from_ddl(mysql_ddl)
...
def extract_notification_indexes_from_ddl(ddl):
    """Extract index definitions from Notification table MySQL DDL"""
    indexes = []
...
def extract_notification_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from Notification table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to Notification - handle multi-word actions like "SET NULL"
...
def convert_notification_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert Notification table MySQL DDL to PostgreSQL DDL with Notification-specific optimizations"""
    print(f" Converting Notification table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_notification_column_definition(line, preserve_case)
            if processed_line:
...
def process_notification_column_definition(line, preserve_case):
    """Process a single column definition for Notification table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for Notification
    conversions = [
...
def create_notification_table(mysql_ddl):
    """Create Notification table in PostgreSQL"""
    postgres_ddl = convert_notification_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_notification_indexes(indexes):
...
def create_notification_foreign_keys(foreign_keys):
    """Create foreign keys for Notification table"""
    if not foreign_keys:
    ...
    for fk in foreign_keys:
        ...
        local_cols = fk['local_columns'].replace('`', '"' if PRESERVE_MYSQL_CASE else '')
        ref_table = f'"{fk["ref_table"]}"' if PRESERVE_MYSQL_CASE else fk["ref_table"].lower()
        ref_cols = fk['ref_columns'].replace('`', '"' if PRESERVE_MYSQL_CASE else '')
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_notification_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '1' or args.full:
        ...
        if create_notification_table(mysql_ddl):
    ...
    if args.phase == '2' or args.full:
        ...
        if not create_notification_indexes(indexes):
            success = False
    
    if args.phase == '3' or args.full:
        print(f" Phase 3: Creating foreign keys for {TABLE_NAME}")
        if not create_notification_foreign_keys(foreign_keys):
            success = False
