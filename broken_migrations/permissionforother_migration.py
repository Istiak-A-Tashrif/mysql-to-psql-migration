...
def get_permissionforother_table_info():
    """Get MySQL table information for PermissionForOther"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `PermissionForOther`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting PermissionForOther table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `PermissionForOther`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'PermissionForOther'"
        ])
        ...
        print(f"Error getting PermissionForOther table info: {e}")
        return None, None, None

def convert_permissionforother_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for PermissionForOther"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?PermissionForOther`?', f"CREATE TABLE "PermissionForOther"\', postgres_ddl)
        ...
        print(f"Error converting PermissionForOther DDL: {e}")
        return None
...
def create_permissionforother_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating PermissionForOther table: {e}")
        return False
...
def create_permissionforother_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating PermissionForOther indexes: {e}")
        return False
...
def create_permissionforother_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating PermissionForOther foreign keys: {e}")
        return False
