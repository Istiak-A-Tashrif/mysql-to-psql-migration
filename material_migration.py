#!/usr/bin/env python3
"""
Material Table Migration Script
=============================

This script provides a complete 3-phase migration approach specifically for the Material table:
...
- Handles Material-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for Material table
- Large table with customer/material data

Usage: 
    python material_migration.py --phase=1
    python material_migration.py --phase=2
    python material_migration.py --phase=3
    python material_migration.py --full
    python material_migration.py --verify
"""
...
def extract_material_indexes_from_ddl(ddl):
    """Extract index definitions from Material table MySQL DDL"""
    indexes = []
...
def extract_material_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from Material table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to Material - handle multi-word actions like "SET NULL"
...
def convert_material_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_material_column_definition(line, preserve_case)
            if processed_line:
...
def process_material_column_definition(line, preserve_case):
    """Process a single column definition for Material table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for Material
    conversions = [
...
def create_material_table(mysql_ddl):
    """Create Material table in PostgreSQL"""
    postgres_ddl = convert_material_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_material_table_info()
    if not mysql_ddl:
        return
