...
def get_invoicephoto_table_info():
    """Get MySQL table information for InvoicePhoto"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `InvoicePhoto`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting InvoicePhoto table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `InvoicePhoto`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'InvoicePhoto'"
        ])
        ...
        print(f"Error getting InvoicePhoto table info: {e}")
        return None, None, None

def convert_invoicephoto_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for InvoicePhoto"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?InvoicePhoto`?', f"CREATE TABLE "InvoicePhoto"\', postgres_ddl)
        ...
        print(f"Error converting InvoicePhoto DDL: {e}")
        return None
...
def create_invoicephoto_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating InvoicePhoto table: {e}")
        return False
...
def create_invoicephoto_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating InvoicePhoto indexes: {e}")
        return False
...
def create_invoicephoto_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating InvoicePhoto foreign keys: {e}")
        return False
