...
def get_leadtags_table_info():
    """Get MySQL table information for LeadTags"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `LeadTags`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting LeadTags table structure: {result.stderr}")
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'LeadTags'"
        ])
...
')
        ...
        print(f"Error getting LeadTags table info: {e}")
        return None, None, None

def convert_leadtags_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for LeadTags"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?LeadTags`?', f'CREATE TABLE "LeadTags"', postgres_ddl)
        ...
        print(f"Error converting LeadTags DDL: {e}")
        return None
...
def create_leadtags_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating LeadTags table: {e}")
        return False
...
def create_leadtags_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating LeadTags indexes: {e}")
        return False
...
def create_leadtags_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating LeadTags foreign keys: {e}")
        return False
