...
This script provides a complete 3-phase migration approach specifically for the InvoiceAutomationRule table:
...
- Handles InvoiceAutomationRule-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for InvoiceAutomationRule table
- Large table with customer/invoiceautomationrule data

Usage: 
    python invoiceautomationrule_migration.py --phase=1
    python invoiceautomationrule_migration.py --phase=2
    python invoiceautomationrule_migration.py --phase=3
    python invoiceautomationrule_migration.py --full
    python invoiceautomationrule_migration.py --verify
"""
...
def get_invoiceautomationrule_table_info():
    """Get complete InvoiceAutomationRule table information from MySQL including constraints"""
    ...
    indexes = extract_invoiceautomationrule_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_invoiceautomationrule_foreign_keys_from_ddl(mysql_ddl)
...
def extract_invoiceautomationrule_indexes_from_ddl(ddl):
    """Extract index definitions from InvoiceAutomationRule table MySQL DDL"""
    indexes = []
    ...
    for match in matches:
        ...
        indexes.append({
            ...
            'table': 'InvoiceAutomationRule'
        })
    
    return indexes

def extract_invoiceautomationrule_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from InvoiceAutomationRule table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to InvoiceAutomationRule - handle multi-word actions like "SET NULL"
...
def convert_invoiceautomationrule_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert InvoiceAutomationRule table MySQL DDL to PostgreSQL DDL with InvoiceAutomationRule-specific optimizations"""
    print(f" Converting InvoiceAutomationRule table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_invoiceautomationrule_column_definition(line, preserve_case)
            if processed_line:
...
def process_invoiceautomationrule_column_definition(line, preserve_case):
    """Process a single column definition for InvoiceAutomationRule table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for InvoiceAutomationRule
    conversions = [
    ...
    # Handle specific InvoiceAutomationRule issues: make first_name nullable to handle empty strings
...
def create_invoiceautomationrule_table(mysql_ddl):
    """Create InvoiceAutomationRule table in PostgreSQL"""
    postgres_ddl = convert_invoiceautomationrule_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_invoiceautomationrule_indexes(indexes):
    """Create indexes for InvoiceAutomationRule table"""
    if not indexes:
...
def create_invoiceautomationrule_foreign_keys(foreign_keys):
    """Create foreign keys for InvoiceAutomationRule table"""
    if not foreign_keys:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_invoiceautomationrule_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '1' or args.full:
        ...
        if not create_invoiceautomationrule_table(mysql_ddl):
            success = False
        else:
    ...
    if args.phase == '2' or args.full:
        ...
        if not create_invoiceautomationrule_indexes(indexes):
            success = False
    
    if args.phase == '3' or args.full:
        print(f" Phase 3: Creating foreign keys for {TABLE_NAME}")
        if not create_invoiceautomationrule_foreign_keys(foreign_keys):
            success = False
