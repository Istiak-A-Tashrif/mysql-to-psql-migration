...
def get_message_table_info():
    """Get MySQL table information for Message"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `Message`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting Message table structure: {result.stderr}")
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'Message'"
        ])
...
')
        ...
        print(f"Error getting Message table info: {e}")
        return None, None, None

def convert_message_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for Message"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?Message`?', f'CREATE TABLE "Message"', postgres_ddl)
        ...
        print(f"Error converting Message DDL: {e}")
        return None
...
def create_message_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating Message foreign keys: {e}")
        return False
