#!/usr/bin/env python3
"""
Task Table Migration Script
=============================

This script provides a complete 3-phase migration approach specifically for the Task table:
...
- Handles Task-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for Task table
- Large table with customer/task data

Usage: 
    python task_migration.py --phase=1
    python task_migration.py --phase=2
    python task_migration.py --phase=3
    python task_migration.py --full
    python task_migration.py --verify
"""
...
def get_task_table_info():
    """Get complete Task table information from MySQL including constraints"""
    ...
    indexes = extract_task_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_task_foreign_keys_from_ddl(mysql_ddl)
...
def extract_task_indexes_from_ddl(ddl):
    """Extract index definitions from Task table MySQL DDL"""
    indexes = []
...
def extract_task_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from Task table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to Task - handle multi-word actions like "SET NULL"
...
def convert_task_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert Task table MySQL DDL to PostgreSQL DDL with Task-specific optimizations"""
    print(f"🔄 Converting Task table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_task_column_definition(line, preserve_case)
            if processed_line:
...
def process_task_column_definition(line, preserve_case):
    """Process a single column definition for Task table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for Task
    conversions = [
        ...
        (r'\btinyblob\b', 'BYTEA'),
        # Task-specific ENUM handling
        (r"enum\('Low','Medium','High'\)", 'task_priority_enum'),
    ]
...
def create_task_table(mysql_ddl):
    """Create Task table in PostgreSQL"""
    postgres_ddl = convert_task_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_task_indexes(indexes):
...
def create_task_foreign_keys(foreign_keys):
    """Create foreign keys for Task table"""
    if not foreign_keys:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_task_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '1' or args.full:
        ...
        if not create_task_table(mysql_ddl):
            success = False
        else:
