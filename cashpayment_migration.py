#!/usr/bin/env python3
"""
CashPayment Table Migration Script
=================================

This script provides a complete 3-phase migration approach specifically for the CashPayment table:
...
- Handles CashPayment-specific data types and constraints
- Manages foreign key dependencies (Payment)
- Creates appropriate indexes for CashPayment table

Usage: 
    python cashpayment_migration.py --phase=1
    python cashpayment_migration.py --phase=2
    python cashpayment_migration.py --phase=3
    python cashpayment_migration.py --full
    python cashpayment_migration.py --verify
"""
...
TABLE_NAME = "CashPayment"

def get_cashpayment_table_info():
    """Get complete CashPayment table information from MySQL including constraints"""
    ...
    indexes = extract_cashpayment_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_cashpayment_foreign_keys_from_ddl(mysql_ddl)
...
def extract_cashpayment_indexes_from_ddl(ddl):
    indexes = []
...
def extract_cashpayment_foreign_keys_from_ddl(ddl):
    foreign_keys = []
...
def convert_cashpayment_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    print(f" Converting CashPayment table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_cashpayment_column_definition(line, preserve_case)
            if processed_line:
...
def process_cashpayment_column_definition(line, preserve_case):
...
def create_cashpayment_table(mysql_ddl):
    postgres_ddl = convert_cashpayment_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_cashpayment_indexes(indexes):
    if not indexes:
...
def create_cashpayment_foreign_keys(foreign_keys):
    if not foreign_keys:
...
def phase1_create_table_and_data():
    ...
    mysql_ddl, indexes, foreign_keys = get_cashpayment_table_info()
    if not mysql_ddl:
        return False
    if not create_cashpayment_table(mysql_ddl):
        return False
    if not export_and_clean_mysql_data(TABLE_NAME):
        return False
    if not import_data_to_postgresql(TABLE_NAME, "CashPayment", PRESERVE_MYSQL_CASE, include_id=True):
        return False
...
def phase2_create_indexes():
    ...
    if mysql_ddl is None:
        return False
    return create_cashpayment_indexes(indexes)
...
def phase3_create_foreign_keys():
    ...
    if mysql_ddl is None:
        return False
    return create_cashpayment_foreign_keys(foreign_keys)
