...
def get_twiliocredentials_table_info():
    """Get MySQL table information for TwilioCredentials"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `TwilioCredentials`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting TwilioCredentials table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `TwilioCredentials`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'TwilioCredentials'"
        ])
        ...
        print(f"Error getting TwilioCredentials table info: {e}")
        return None, None, None

def convert_twiliocredentials_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for TwilioCredentials"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?TwilioCredentials`?', f"CREATE TABLE "TwilioCredentials"\', postgres_ddl)
        ...
        print(f"Error converting TwilioCredentials DDL: {e}")
        return None
...
def create_twiliocredentials_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating TwilioCredentials table: {e}")
        return False
...
def create_twiliocredentials_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating TwilioCredentials indexes: {e}")
        return False
...
def create_twiliocredentials_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating TwilioCredentials foreign keys: {e}")
        return False
