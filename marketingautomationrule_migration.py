...
def get_marketingautomationrule_table_info():
    """Get MySQL table information for MarketingAutomationRule"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `MarketingAutomationRule`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting MarketingAutomationRule table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW INDEX FROM `MarketingAutomationRule`;'
        ])
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'MarketingAutomationRule'"
        ])
...
')
        ...
        print(f"Error getting MarketingAutomationRule table info: {e}")
        return None, None, None

def convert_marketingautomationrule_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for MarketingAutomationRule"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?MarketingAutomationRule`?', f'CREATE TABLE "MarketingAutomationRule"', postgres_ddl)
        ...
        print(f"Error converting MarketingAutomationRule DDL: {e}")
        return None
...
def create_marketingautomationrule_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating MarketingAutomationRule table: {e}")
        return False
...
def create_marketingautomationrule_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating MarketingAutomationRule indexes: {e}")
        return False
...
def create_marketingautomationrule_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating MarketingAutomationRule foreign keys: {e}")
        return False
