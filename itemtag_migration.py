...
def get_itemtag_table_info():
    """Get MySQL table information for ItemTag"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `ItemTag`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting ItemTag table structure: {result.stderr}")
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'ItemTag'"
        ])
...
')
        ...
        print(f"Error getting ItemTag table info: {e}")
        return None, None, None

def convert_itemtag_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for ItemTag"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?ItemTag`?', f'CREATE TABLE "ItemTag"', postgres_ddl)
        ...
        print(f"Error converting ItemTag DDL: {e}")
        return None
...
def create_itemtag_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating ItemTag table: {e}")
        return False
...
def create_itemtag_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating ItemTag indexes: {e}")
        return False
...
def create_itemtag_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating ItemTag foreign keys: {e}")
        return False
