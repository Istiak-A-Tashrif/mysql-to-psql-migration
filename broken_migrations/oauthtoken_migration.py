...
def get_oauthtoken_table_info():
    """Get MySQL table information for OAuthToken"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `OAuthToken`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting OAuthToken table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `OAuthToken`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'OAuthToken'"
        ])
        ...
        print(f"Error getting OAuthToken table info: {e}")
        return None, None, None

def convert_oauthtoken_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for OAuthToken"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?OAuthToken`?', f"CREATE TABLE "OAuthToken"\', postgres_ddl)
        ...
        print(f"Error converting OAuthToken DDL: {e}")
        return None
...
def create_oauthtoken_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating OAuthToken table: {e}")
        return False
...
def create_oauthtoken_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating OAuthToken indexes: {e}")
        return False
...
def create_oauthtoken_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating OAuthToken foreign keys: {e}")
        return False
