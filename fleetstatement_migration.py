...
def get_fleetstatement_table_info():
    """Get MySQL table information for FleetStatement"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `FleetStatement`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting FleetStatement table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW INDEX FROM `FleetStatement`;'
        ])
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'FleetStatement'"
        ])
...
')
        ...
        print(f"Error getting FleetStatement table info: {e}")
        return None, None, None

def convert_fleetstatement_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for FleetStatement"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?FleetStatement`?', f'CREATE TABLE "FleetStatement"', postgres_ddl)
        ...
        print(f"Error converting FleetStatement DDL: {e}")
        return None
...
def create_fleetstatement_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating FleetStatement table: {e}")
        return False
...
def create_fleetstatement_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating FleetStatement indexes: {e}")
        return False
...
def create_fleetstatement_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating FleetStatement foreign keys: {e}")
        return False
