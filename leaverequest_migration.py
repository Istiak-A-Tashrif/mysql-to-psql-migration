...
def get_leaverequest_table_info():
    """Get MySQL table information for LeaveRequest"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `LeaveRequest`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting LeaveRequest table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW INDEX FROM `LeaveRequest`;'
        ])
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'LeaveRequest'"
        ])
...
')
        ...
        print(f"Error getting LeaveRequest table info: {e}")
        return None, None, None

def convert_leaverequest_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for LeaveRequest"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?LeaveRequest`?', f'CREATE TABLE "LeaveRequest"', postgres_ddl)
        ...
        print(f"Error converting LeaveRequest DDL: {e}")
        return None
...
def create_leaverequest_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating LeaveRequest table: {e}")
        return False
...
def create_leaverequest_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating LeaveRequest indexes: {e}")
        return False
...
def create_leaverequest_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating LeaveRequest foreign keys: {e}")
        return False
