...
This script provides a complete 3-phase migration approach specifically for the Invoice table:
...
- Handles Invoice-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for Invoice table
- Large table with customer/invoice data

Usage: 
    python invoice_migration.py --phase=1
    python invoice_migration.py --phase=2
    python invoice_migration.py --phase=3
    python invoice_migration.py --full
    python invoice_migration.py --verify
"""
...
def get_invoice_table_info():
    """Get complete Invoice table information from MySQL including constraints"""
    ...
    indexes = extract_invoice_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_invoice_foreign_keys_from_ddl(mysql_ddl)
...
def extract_invoice_indexes_from_ddl(ddl):
    """Extract index definitions from Invoice table MySQL DDL"""
    indexes = []
...
def extract_invoice_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from Invoice table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to Invoice - handle multi-word actions like "SET NULL"
...
def convert_invoice_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert Invoice table MySQL DDL to PostgreSQL DDL with Invoice-specific optimizations"""
    print(f" Converting Invoice table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_invoice_column_definition(line, preserve_case)
            if processed_line:
...
def process_invoice_column_definition(line, preserve_case):
    """Process a single column definition for Invoice table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for Invoice
    conversions = [
        ...
        (r'\btinyblob\b', 'BYTEA'),
        # Invoice-specific ENUM handling
        (r"enum\('Invoice','Estimate'\)", 'invoice_type_enum'),
    ]
...
def create_invoice_table(mysql_ddl):
    """Create Invoice table in PostgreSQL"""
    postgres_ddl = convert_invoice_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_invoice_indexes(indexes):
...
def create_invoice_foreign_keys(foreign_keys):
    """Create foreign keys for Invoice table"""
    if not foreign_keys:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_invoice_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '1' or args.full:
        ...
        if not create_invoice_table(mysql_ddl):
            success = False
        else:
