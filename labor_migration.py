#!/usr/bin/env python3
"""
Labor Table Migration Script
=============================

This script provides a complete 3-phase migration approach specifically for the Labor table:
...
- Handles Labor-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for Labor table
- Large table with customer/labor data

Usage: 
    python labor_migration.py --phase=1
    python labor_migration.py --phase=2
    python labor_migration.py --phase=3
    python labor_migration.py --full
    python labor_migration.py --verify
"""
...
def get_labor_table_info():
    """Get complete Labor table information from MySQL including constraints"""
    ...
    indexes = extract_labor_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_labor_foreign_keys_from_ddl(mysql_ddl)
...
def extract_labor_indexes_from_ddl(ddl):
    """Extract index definitions from Labor table MySQL DDL"""
    indexes = []
...
def extract_labor_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from Labor table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to Labor - handle multi-word actions like "SET NULL"
...
def convert_labor_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert Labor table MySQL DDL to PostgreSQL DDL with Labor-specific optimizations"""
    print(f"🔄 Converting Labor table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_labor_column_definition(line, preserve_case)
            if processed_line:
...
def process_labor_column_definition(line, preserve_case):
    """Process a single column definition for Labor table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for Labor
    conversions = [
...
def create_labor_table(mysql_ddl):
    """Create Labor table in PostgreSQL"""
    postgres_ddl = convert_labor_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_labor_indexes(indexes):
    """Create indexes for Labor table"""
    if not indexes:
...
def create_labor_foreign_keys(foreign_keys):
    """Create foreign keys for Labor table"""
    if not foreign_keys:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_labor_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '1' or args.full:
        ...
        if not create_labor_table(mysql_ddl):
            success = False
        else:
    ...
    if args.phase == '2' or args.full:
        ...
        if not create_labor_indexes(indexes):
            success = False
    
    if args.phase == '3' or args.full:
        print(f"🔗 Phase 3: Creating foreign keys for {TABLE_NAME}")
        if not create_labor_foreign_keys(foreign_keys):
            success = False
