...
def verify_data_migration(table_name, preserve_case=True):
    ...
    # Use backticks for MySQL to handle reserved words
    mysql_cmd = f'docker exec mysql_source mysql -u mysql -pmysql source_db -e "SELECT COUNT(*) FROM `{table_name}`;"'
    mysql_result = run_command(mysql_cmd)
    
    # PostgreSQL table name handling
    pg_table_name = f'"{table_name}"' if preserve_case else table_name.lower()
    postgres_cmd = f'docker exec postgres_target psql -U postgres -d target_db -t -c "SELECT COUNT(*) FROM {pg_table_name};"'
    postgres_result = run_command(postgres_cmd)
    ...
    if mysql_result and mysql_result.returncode == 0:
        ...
        if len(lines) >= 2:
            try:
                ...
                print(f"Could not parse MySQL count for {table_name}")
                return False
    else:
    ...
    if postgres_result and postgres_result.returncode == 0:
        try:
            postgres_count = int(postgres_result.stdout.strip())
        except ValueError:
            print(f"Could not parse PostgreSQL count for {table_name}")
            return False
    else:
    ...
    print(f"PostgreSQL {pg_table_name}: {postgres_count} records")
