#!/usr/bin/env python3
"""
refunds Table Migration Script
=============================

This script provides a complete 3-phase migration approach specifically for the refunds table:
...
- Handles refunds-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for refunds table
- Large table with customer/refunds data

Usage: 
    python refunds_migration.py --phase=1
    python refunds_migration.py --phase=2
    python refunds_migration.py --phase=3
    python refunds_migration.py --full
    python refunds_migration.py --verify
"""
...
def get_refunds_table_info():
    """Get complete refunds table information from MySQL including constraints"""
    ...
    indexes = extract_refunds_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_refunds_foreign_keys_from_ddl(mysql_ddl)
...
def extract_refunds_indexes_from_ddl(ddl):
    """Extract index definitions from refunds table MySQL DDL"""
    indexes = []
...
def extract_refunds_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from refunds table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to refunds - handle multi-word actions like "SET NULL"
...
def convert_refunds_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert refunds table MySQL DDL to PostgreSQL DDL with refunds-specific optimizations"""
    print(f" Converting refunds table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_refunds_column_definition(line, preserve_case)
            if processed_line:
...
def process_refunds_column_definition(line, preserve_case):
    """Process a single column definition for refunds table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for refunds
    conversions = [
    ...
    # Handle specific refunds issues: make first_name nullable to handle empty strings
...
def create_refunds_table(mysql_ddl):
    """Create refunds table in PostgreSQL"""
    postgres_ddl = convert_refunds_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_refunds_indexes(indexes):
...
def create_refunds_foreign_keys(foreign_keys):
    """Create foreign keys for refunds table"""
    if not foreign_keys:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_refunds_table_info()
    if not mysql_ddl:
        return
    ...
    if args.phase == '1' or args.full:
        ...
        if not create_refunds_table(mysql_ddl):
            success = False
        else:
    ...
    if args.phase == '2' or args.full:
        ...
        if not create_refunds_indexes(indexes):
            success = False
    
    if args.phase == '3' or args.full:
        print(f" Phase 3: Creating foreign keys for {TABLE_NAME}")
        if not create_refunds_foreign_keys(foreign_keys):
            success = False
