#!/usr/bin/env python3
"""
Technician Table Migration Script
=============================

This script provides a complete 3-phase migration approach specifically for the Technician table:
...
- Handles Technician-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for Technician table
- Large table with customer/technician data

Usage: 
    python technician_migration.py --phase=1
    python technician_migration.py --phase=2
    python technician_migration.py --phase=3
    python technician_migration.py --full
    python technician_migration.py --verify
"""
...
def extract_technician_indexes_from_ddl(ddl):
    """Extract index definitions from Technician table MySQL DDL"""
    indexes = []
...
def extract_technician_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from Technician table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to Technician - handle multi-word actions like "SET NULL"
...
def convert_technician_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_technician_column_definition(line, preserve_case)
            if processed_line:
...
def process_technician_column_definition(line, preserve_case):
    """Process a single column definition for Technician table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for Technician
    conversions = [
        ...
        (r'\bbigint\([^)]+\)\b', 'BIGINT'),
        (r'\bint\b', 'INTEGER'),
        (r'\bvarchar\([^)]+\)\b', 'TEXT'),  # Use TEXT instead of VARCHAR to avoid length issues
        (r'\btext\b', 'TEXT'),
        (r'\blongtext\b', 'TEXT'),
        ...
        (r'\btinyblob\b', 'BYTEA'),
        # Technician-specific ENUM handling
        (r"enum\('Low','Medium','High'\)", 'technician_priority_enum'),
    ]
    
    for pattern, replacement in conversions:
        line = re.sub(pattern, replacement, line, flags=re.IGNORECASE)
    
    # Additional specific fixes for Technician table issues
    if '"new_note"' in line:
        line = re.sub(r'varchar\([^)]+\)', 'TEXT', line, flags=re.IGNORECASE)
    
    # Handle problematic NOT NULL constraints for Technician table
    # Remove NOT NULL from columns that may have empty values in MySQL
    if 'service_id' in line and 'NOT NULL' in line:
        line = line.replace('NOT NULL', 'DEFAULT NULL')
    if 'invoice_id' in line and 'NOT NULL' in line:
        line = line.replace('NOT NULL', 'DEFAULT NULL')
    if 'invoice_item_id' in line and 'NOT NULL' in line:
        line = line.replace('NOT NULL', 'DEFAULT NULL')
...
def create_technician_table(mysql_ddl):
    """Create Technician table in PostgreSQL"""
    postgres_ddl = convert_technician_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_technician_table_info()
    if not mysql_ddl:
        return
