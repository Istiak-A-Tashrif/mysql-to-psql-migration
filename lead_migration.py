#!/usr/bin/env python3
"""
Lead Table Migration Script
=============================

This script provides a complete 3-phase migration approach specifically for the Lead table:
...
- Handles Lead-specific data types and constraints
- Manages foreign key dependencies (Company, User, Source, VehicleColor)
- Creates appropriate indexes for Lead table
- Large table with customer/lead data

Usage: 
    python lead_migration.py --phase=1
    python lead_migration.py --phase=2
    python lead_migration.py --phase=3
    python lead_migration.py --full
    python lead_migration.py --verify
"""
...
def get_lead_table_info():
    ...
    indexes = extract_lead_indexes_from_ddl(mysql_ddl)
    foreign_keys = extract_lead_foreign_keys_from_ddl(mysql_ddl)
...
def extract_lead_indexes_from_ddl(ddl):
    """Extract index definitions from Lead table MySQL DDL"""
    indexes = []
...
def extract_lead_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from Lead table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to Lead - handle multi-word actions like "SET NULL"
...
def convert_lead_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    """Convert Lead table MySQL DDL to PostgreSQL DDL with Lead-specific optimizations"""
    print(f"🔄 Converting Lead table MySQL DDL to PostgreSQL (constraints: {include_constraints}, preserve_case: {preserve_case})...")
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_lead_column_definition(line, preserve_case)
            if processed_line:
...
def process_lead_column_definition(line, preserve_case):
    """Process a single column definition for Lead table"""
...
def create_lead_table(mysql_ddl):
    """Create Lead table in PostgreSQL"""
    postgres_ddl = convert_lead_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def create_lead_indexes(indexes):
...
def create_lead_foreign_keys(foreign_keys):
    """Create foreign keys for Lead table"""
    if not foreign_keys:
...
def main():
    ...
    mysql_ddl, indexes, foreign_keys = get_lead_table_info()
    if not mysql_ddl:
        return
