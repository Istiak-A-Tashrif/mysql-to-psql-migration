#!/usr/bin/env python3
"""
inventoryWirehouseProduct Table Migration Script
===============================================

This script provides a complete 3-phase migration approach specifically for the inventoryWirehouseProduct table:
...
- Handles inventoryWirehouseProduct-specific data types and constraints
- Manages foreign key dependencies
- Creates appropriate indexes for inventoryWirehouseProduct table
- Large table with 3,090+ inventory warehouse product records
- Handles inventory-specific data like quantities, locations, and product relationships

Usage: 
    python inventorywirehouseproduct_migration.py --phase=1
    python inventorywirehouseproduct_migration.py --phase=2
    python inventorywirehouseproduct_migration.py --phase=3
    python inventorywirehouseproduct_migration.py --full
    python inventorywirehouseproduct_migration.py --verify
"""
...
def extract_inventorywirehouseproduct_indexes_from_ddl(ddl):
    """Extract index definitions from inventoryWirehouseProduct table MySQL DDL"""
    indexes = []
    ...
    for match in matches:
        ...
        indexes.append({
            ...
            'table': 'inventoryWirehouseProduct'
        })
    
    return indexes

def extract_inventorywirehouseproduct_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from inventoryWirehouseProduct table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to inventoryWirehouseProduct - handle multi-word actions like "SET NULL"
...
def convert_inventorywirehouseproduct_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_inventorywirehouseproduct_column_definition(line, preserve_case)
            if processed_line:
...
def process_inventorywirehouseproduct_column_definition(line, preserve_case):
    """Process a single column definition for inventoryWirehouseProduct table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # MySQL to PostgreSQL type conversions for inventoryWirehouseProduct
    conversions = [
...
def create_inventorywirehouseproduct_table(mysql_ddl):
    """Create inventoryWirehouseProduct table in PostgreSQL"""
    postgres_ddl = convert_inventorywirehouseproduct_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def phase1_create_table_and_data():
    ...
    # Create table
    if not create_inventorywirehouseproduct_table(mysql_ddl):
        return False
    ...
    if not import_data_to_postgresql(TABLE_NAME, "inventoryWirehouseProduct", PRESERVE_MYSQL_CASE, include_id=True):
        return False
...
def phase2_create_indexes():
    ...
    return create_inventorywirehouseproduct_indexes(indexes)
...
def phase3_create_foreign_keys():
    ...
    return create_inventorywirehouseproduct_foreign_keys(foreign_keys)
