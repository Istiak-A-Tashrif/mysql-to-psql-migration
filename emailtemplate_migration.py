...
def get_emailtemplate_table_info():
    """Get MySQL table information for EmailTemplate"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `EmailTemplate`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting EmailTemplate table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW INDEX FROM `EmailTemplate`;'
        ])
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'EmailTemplate'"
        ])
...
')
        ...
        print(f"Error getting EmailTemplate table info: {e}")
        return None, None, None

def convert_emailtemplate_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for EmailTemplate"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?EmailTemplate`?', f'CREATE TABLE "EmailTemplate"', postgres_ddl)
        ...
        print(f"Error converting EmailTemplate DDL: {e}")
        return None
...
def create_emailtemplate_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating EmailTemplate indexes: {e}")
        return False
...
def create_emailtemplate_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating EmailTemplate foreign keys: {e}")
        return False
