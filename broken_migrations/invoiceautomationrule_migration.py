...
def get_invoiceautomationrule_table_info():
    """Get MySQL table information for InvoiceAutomationRule"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `InvoiceAutomationRule`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting InvoiceAutomationRule table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW INDEX FROM `InvoiceAutomationRule`;
        ])
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'InvoiceAutomationRule'"
        ])
        ...
        print(f"Error getting InvoiceAutomationRule table info: {e}")
        return None, None, None

def convert_invoiceautomationrule_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for InvoiceAutomationRule"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?InvoiceAutomationRule`?', f"CREATE TABLE "InvoiceAutomationRule"\', postgres_ddl)
        ...
        print(f"Error converting InvoiceAutomationRule DDL: {e}")
        return None
...
def create_invoiceautomationrule_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating InvoiceAutomationRule table: {e}")
        return False
...
def create_invoiceautomationrule_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating InvoiceAutomationRule indexes: {e}")
        return False
...
def create_invoiceautomationrule_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating InvoiceAutomationRule foreign keys: {e}")
        return False
