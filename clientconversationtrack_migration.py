#!/usr/bin/env python3
"""
ClientConversationTrack Table Migration Script
==============================================

This script provides a complete 3-phase migration approach specifically for the ClientConversationTrack table:
...
- Handles ClientConversationTrack-specific data types and constraints
- Manages foreign key dependencies
- Creates appropriate indexes for ClientConversationTrack table
- Medium table with 1,828+ conversation tracking records
- Handles conversation tracking data and communication logs

Usage: 
    python clientconversationtrack_migration.py --phase=1
    python clientconversationtrack_migration.py --phase=2
    python clientconversationtrack_migration.py --phase=3
    python clientconversationtrack_migration.py --full
    python clientconversationtrack_migration.py --verify
"""
...
TABLE_NAME = "ClientConversationTrack"
...
def extract_clientconversationtrack_indexes_from_ddl(ddl):
    """Extract index definitions from ClientConversationTrack table MySQL DDL"""
    indexes = []
    ...
    for match in matches:
        ...
        indexes.append({
            ...
            'table': 'ClientConversationTrack'
        })
    
    return indexes

def extract_clientconversationtrack_foreign_keys_from_ddl(ddl):
    """Extract foreign key definitions from ClientConversationTrack table MySQL DDL"""
    foreign_keys = []
    
    # Pattern for CONSTRAINT FOREIGN KEY specific to ClientConversationTrack - handle multi-word actions like "SET NULL"
...
def convert_clientconversationtrack_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=True):
    ...
    for line in columns_part.split(',\n'):
        ...
        if not (line.startswith('PRIMARY KEY') or line.startswith('KEY') or 
            ...
            processed_line = process_clientconversationtrack_column_definition(line, preserve_case)
            if processed_line:
...
def process_clientconversationtrack_column_definition(line, preserve_case):
    """Process a single column definition for ClientConversationTrack table"""
    # Remove backticks and handle MySQL-specific types
    line = line.replace('`', '"' if preserve_case else '')
    
    # Convert message columns to TEXT type to avoid varchar length errors
...
def create_clientconversationtrack_table(mysql_ddl):
    """Create ClientConversationTrack table in PostgreSQL"""
    postgres_ddl = convert_clientconversationtrack_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=False, preserve_case=PRESERVE_MYSQL_CASE)
    if not postgres_ddl:
        return False
...
def phase1_create_table_and_data():
    ...
    if not import_data_to_postgresql(TABLE_NAME, "ClientConversationTrack", PRESERVE_MYSQL_CASE, include_id=True):
        return False
...
def phase2_create_indexes():
    ...
    return create_clientconversationtrack_indexes(indexes)
...
def phase3_create_foreign_keys():
    ...
    return create_clientconversationtrack_foreign_keys(foreign_keys)
