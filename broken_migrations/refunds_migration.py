...
def get_refunds_table_info():
    """Get MySQL table information for refunds"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SHOW CREATE TABLE `refunds`;"
        ])
        
        if result.returncode != 0:
            print(f"Error getting refunds table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'refunds'"
        ])
        ...
        print(f"Error getting refunds table info: {e}")
        return None, None, None

def convert_refunds_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for refunds"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?refunds`?', f"CREATE TABLE "refunds"\', postgres_ddl)
        ...
        print(f"Error converting refunds DDL: {e}")
        return None
...
def create_refunds_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating refunds indexes: {e}")
        return False
...
def create_refunds_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating refunds foreign keys: {e}")
        return False
