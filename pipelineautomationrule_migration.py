...
def get_pipelineautomationrule_table_info():
    """Get MySQL table information for PipelineAutomationRule"""
    try:
        # Get CREATE TABLE statement
        result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW CREATE TABLE `PipelineAutomationRule`;'
        ])
        
        if result.returncode != 0:
            print(f"Error getting PipelineAutomationRule table structure: {result.stderr}")
...
'.join(create_table_lines[1:])  # Skip header
        ...
        index_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f'SHOW INDEX FROM `PipelineAutomationRule`;'
        ])
...
')
        ...
        fk_result = run_command([
            'docker', 'exec', '-i', 'mysql_source', 
            'mysql', '-u', 'root', '-prootpass', 'source_db', 
            '-e', f"SELECT CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = 'source_db' AND TABLE_NAME = 'PipelineAutomationRule'"
        ])
...
')
        ...
        print(f"Error getting PipelineAutomationRule table info: {e}")
        return None, None, None

def convert_pipelineautomationrule_mysql_to_postgresql_ddl(mysql_ddl, include_constraints=True, preserve_case=True):
    """Convert MySQL DDL to PostgreSQL DDL for PipelineAutomationRule"""
    try:
        # Basic conversions
        ...
        if preserve_case:
            # Quote table name
            postgres_ddl = re.sub(r'CREATE TABLE `?PipelineAutomationRule`?', f'CREATE TABLE "PipelineAutomationRule"', postgres_ddl)
        ...
        print(f"Error converting PipelineAutomationRule DDL: {e}")
        return None
...
def create_pipelineautomationrule_table(mysql_ddl):
    ...
    try:
        ...
        print(f"Error creating PipelineAutomationRule table: {e}")
        return False
...
def create_pipelineautomationrule_indexes(indexes):
    ...
    try:
        ...
        print(f"Error creating PipelineAutomationRule indexes: {e}")
        return False
...
def create_pipelineautomationrule_foreign_keys(foreign_keys):
    ...
    try:
        ...
        print(f"Error creating PipelineAutomationRule foreign keys: {e}")
        return False
